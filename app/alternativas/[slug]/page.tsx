...
export async function generateStaticParams() {
  return ALL_ALTERNATIVE_SLUGS_ES.map((slug) => ({ slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const canonicalUrl = `${SITE_URL}/alternativas/${data.slug}`;
  const enSlug = getEquivalentComparisonSlug(data.slug, 'es');
  const enUrl = `${SITE_URL}/en/alternatives/${enSlug}`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 2,
        name: 'Alternativas',
        item: `${SITE_URL}/alternativas/alternativa-privada-a-ilovepdf`,
      },
      {
        '@type': 'ListItem',
        position: 3,
