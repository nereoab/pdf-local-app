  ...
  try {
    ...
    if (engine === 'cloudconvert') {
      try {
        ...
        console.warn(
          '[PowerPoint-to-PDF] CloudConvert API error, attempting Adobe fallback:',
          ccErr,
        );
      }
    }
    ...
    if (process.env.PDF_SERVICES_CLIENT_ID && process.env.PDF_SERVICES_CLIENT_SECRET) {
      try {
        ...
        console.warn(
          '[PowerPoint-to-PDF] Adobe API error, attempting CloudConvert fallback:',
          adobeErr,
        );
      }
    }
    ...
    if (engine !== 'cloudconvert') {
      try {
        ...
        console.warn('[PowerPoint-to-PDF] CloudConvert fallback error:', ccErr2);
      }
    }
