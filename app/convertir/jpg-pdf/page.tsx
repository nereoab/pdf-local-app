        ...
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
          ...
          <div>
            <div className="flex items-center gap-2">
              <Link href="/convertir" className="text-xs font-bold text-slate-400 hover:text-amber-400 transition-colors">
                {isEs ? 'Convertir PDF' : 'Convert PDF'}
              </Link>
              <span className="text-slate-600 text-xs">/</span>
              <span className="text-amber-400 text-xs font-bold">{isEs ? 'JPG a PDF' : 'JPG to PDF'}</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              {isEs ? 'CONVERTIR IMAGEN (JPG) A PDF' : 'CONVERT IMAGE (JPG) TO PDF'}
            </h1>
          </div>
        </div>
