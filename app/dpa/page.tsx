      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12 font-mono">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-900 border border-white/10 text-zinc-300 text-xs font-semibold mb-4">
            ...
            {isEs ? '010 / ACUERDO DE PROCESAMIENTO DE DATOS (DPA)' : '010 / DATA PROCESSING AGREEMENT (DPA)'}
          </div>
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-10 shadow-2xl space-y-8 font-sans" role="article" aria-label={isEs ? 'Acuerdo de procesamiento de datos completo' : 'Full data processing agreement'}>
          
          <section className="space-y-3" aria-labelledby="dpa-1">
            <h2 id="dpa-1" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
            ...
            <div className="space-y-2 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p><strong className="text-white">{isEs ? 'Responsable del Tratamiento (Data Controller):' : 'Data Controller:'}</strong> {isEs ? 'El usuario de PDFBlack que carga, procesa, o manipula archivos PDF que contienen datos personales.' : 'The PDFBlack user who uploads, processes, or manipulates PDF files containing personal data.'}</p>
              <p><strong className="text-white">{isEs ? 'Encargado del Tratamiento (Data Processor):' : 'Data Processor:'}</strong> {isEs ? 'PDFBlack, proveedor del servicio de herramientas de procesamiento de PDF en línea.' : 'PDFBlack, provider of the online PDF processing tools service.'}</p>
            </div>
          </section>

          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-2">
            <h2 id="dpa-2" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '2. Objeto y Naturaleza del Tratamiento' : '2. Subject Matter and Nature of Processing'}
            </h2>
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-3">
            <h2 id="dpa-3" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-4">
            <h2 id="dpa-4" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <ShieldCheck className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '4. Medidas Técnicas y Organizativas de Seguridad' : '4. Technical and Organizational Security Measures'}
            </h2>
            <div className="space-y-2 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p>
                {isEs
              ...
              <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm" aria-label={isEs ? 'Medidas de seguridad implementadas' : 'Implemented security measures'}>
                <li>{isEs ? 'Procesamiento 100% local en RAM del navegador (sin persistencia en disco ni transmisión a servidores).' : '100% local processing in browser RAM (no disk persistence or server transmission).'}</li>
                <li>{isEs ? 'Cifrado AES-256 conforme a ISO 32000-2:2020 para protección de archivos PDF.' : 'AES-256 encryption per ISO 32000-2:2020 for PDF file protection.'}</li>
                <li>{isEs ? 'Aislamiento de procesos mediante Web Workers (sin acceso al DOM principal ni a otras pestañas).' : 'Process isolation via Web Workers (no access to main DOM or other tabs).'}</li>
                <li>{isEs ? 'Comunicaciones HTTPS/TLS 1.3 con HSTS y Content Security Policy.' : 'HTTPS/TLS 1.3 communications with HSTS and Content Security Policy.'}</li>
                <li>{isEs ? 'Censura binaria real que elimina físicamente los datos del código del documento.' : 'True binary redaction that physically removes data from document code.'}</li>
                <li>{isEs ? 'No se utilizan subencargados del tratamiento externos.' : 'No external sub-processors are used.'}</li>
              </ul>
            </div>
          </section>

          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-5">
            <h2 id="dpa-5" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '5. Obligaciones del Encargado del Tratamiento' : '5. Obligations of the Data Processor'}
            </h2>
            <div className="space-y-1 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p>{isEs ? 'PDFBlack, como Encargado del Tratamiento, se obliga a:' : 'PDFBlack, as Data Processor, undertakes to:'}</p>
              <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm" aria-label={isEs ? 'Obligaciones del encargado' : 'Processor obligations'}>
                <li>{isEs ? 'Tratar los datos personales únicamente según las instrucciones documentadas del Responsable (el usuario).' : 'Process personal data only according to the documented instructions of the Controller (the user).'}</li>
                <li>{isEs ? 'No comunicar los datos personales a terceros, salvo obligación legal.' : 'Not disclose personal data to third parties, except by legal obligation.'}</li>
                <li>{isEs ? 'Garantizar que las personas autorizadas para tratar datos se han comprometido a respetar la confidencialidad.' : 'Ensure that persons authorized to process data have committed to confidentiality.'}</li>
                <li>{isEs ? 'Asistir al Responsable en el cumplimiento de sus obligaciones de seguridad, notificación de brechas y evaluaciones de impacto.' : 'Assist the Controller in fulfilling security, breach notification, and impact assessment obligations.'}</li>
                <li>{isEs ? 'Eliminar todos los datos personales al finalizar la sesión de tratamiento (cierre de pestaña).' : 'Delete all personal data upon completion of the processing session (tab closure).'}</li>
                <li>{isEs ? 'Notificar al Responsable cualquier violación de seguridad sin dilación indebida (en un plazo máximo de 72 horas).' : 'Notify the Controller of any security breach without undue delay (within 72 hours).'}</li>
              </ul>
            </div>
          </section>

          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-6">
            <h2 id="dpa-6" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-7">
            <h2 id="dpa-7" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="dpa-8">
            <h2 id="dpa-8" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <FileText className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '8. Aceptación y Vigencia' : '8. Acceptance and Validity'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
                ? 'El presente DPA entra en vigor en el momento en que el usuario carga un archivo PDF en PDFBlack y permanece vigente mientras dure la sesión de procesamiento. La aceptación de este DPA se produce por el mero uso de la plataforma, sin necesidad de firma adicional. Para obtener una copia firmada, contacta a dpa@pdfblack.com.'
                : 'This DPA enters into force at the moment the user uploads a PDF file to PDFBlack and remains in effect for the duration of the processing session. Acceptance of this DPA occurs by the mere use of the platform, without the need for additional signature. To obtain a signed copy, contact dpa@pdfblack.com.'}
            </p>
          </section>
