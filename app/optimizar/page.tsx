...
import { 
  Minimize2, Lock, Unlock, Wrench,
  UploadCloud, FilePlus, FileText, X, Home, ShieldCheck, Zap, ArrowRight
} from "lucide-react";
import { useFileStore } from "../../store/useFileStore"; 

const herramientasOptimizar = [
  { id: "comprimir", nombre: "Comprimir PDF", descripcion: "Reduce el peso del archivo sin perder calidad visual.", icono: Minimize2, href: "/optimizar/comprimir", color: "text-emerald-400", glow: "bg-emerald-500/20" },
  { id: "proteger", nombre: "Proteger PDF", descripcion: "Añade contraseña y encriptación AES-256.", icono: Lock, href: "/optimizar/proteger", color: "text-blue-400", glow: "bg-blue-500/20", isPremium: true },
  { id: "desbloquear", nombre: "Desbloquear PDF", descripcion: "Quita la contraseña de un PDF (si la conoces).", icono: Unlock, href: "/optimizar/desbloquear", color: "text-orange-400", glow: "bg-orange-500/20" },
  { id: "reparar", nombre: "Reparar PDF", descripcion: "Intenta recuperar datos de un PDF dañado o corrupto.", icono: Wrench, href: "/optimizar/reparar", color: "text-purple-400", glow: "bg-purple-500/20" }
];
    ...
    <div className={`w-full px-4 sm:px-6 lg:px-8 pb-24 flex flex-col items-center justify-start relative min-h-[calc(100vh-80px)] bg-[#030712] transition-all duration-700 ${file ? 'pt-8' : 'pt-32'}`}>
      ...
      <div className="w-full max-w-5xl relative">
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="home-view" exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="relative z-10">
              <div className="text-center mb-10">
                <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">
                  Optimizar <span className="text-purple-400">PDF</span>
                </h1>
                <p className="text-gray-400 text-lg max-w-2xl mx-auto">
                  Reduce el tamaño o asegura tus documentos con encriptación avanzada.
                </p>
              </div>

              <div onClick={() => fileInputRef.current?.click()} className="w-full mb-12 bg-white/[0.02] hover:bg-white/[0.04] border border-white/10 hover:border-purple-500/50 border-dashed rounded-3xl p-12 md:p-20 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300 group shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                <div className="bg-purple-500/10 p-6 rounded-full mb-6 group-hover:scale-110 group-hover:bg-purple-500/20 transition-all duration-300 shadow-[0_0_30px_rgba(168,85,247,0.15)]">
                  <UploadCloud className="w-16 h-16 text-purple-400" strokeWidth={1.5} />
                </div>
          ...
          {isUploading && (
            <motion.div key="uploading-view" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full bg-black/60 border border-purple-500/30 rounded-3xl p-12 shadow-[0_0_40px_rgba(168,85,247,0.15)] mt-10 relative z-[50]">
              <div className="max-w-md mx-auto">
                <div className="flex justify-between items-end mb-4">
                  <div className="text-left">
                    <h3 className="text-white font-bold text-xl flex items-center gap-2">Procesando documento...</h3>
                    <p className="text-purple-400/80 text-sm flex items-center gap-1 mt-1"><ShieldCheck className="w-4 h-4" /> Entorno local seguro</p>
                  </div>
                ...
                <div className="w-full bg-gray-900 rounded-full h-3 overflow-hidden border border-white/10 shadow-inner">
                  <motion.div className="bg-gradient-to-r from-purple-500 to-fuchsia-500 h-full rounded-full relative" initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} transition={{ ease: "linear", duration: 0.1 }}>
          ...
          {file && pdfUrl && !isUploading && (
            <motion.div key="preview-view" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full flex flex-col items-center relative z-[50]">
              ...
              <div className="w-48 sm:w-56 bg-white/[0.02] border border-purple-500/30 rounded-2xl overflow-hidden shadow-[0_0_50px_rgba(168,85,247,0.15)] mb-6 flex flex-col relative group">
                <div className="bg-purple-950/60 backdrop-blur-xl border-b border-purple-500/30 p-2 flex justify-between items-center z-10">
                  <div className="flex items-center gap-2 overflow-hidden">
                    ...
                    <div className="flex flex-col overflow-hidden">
                      ...
                      <span className="text-purple-400/80 text-[8px] font-medium">{formatFileSize(file.size)}</span>
                    </div>
                  </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mb-6 flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold uppercase tracking-widest text-purple-400">¿Qué deseas hacer con este archivo?</h3>
              </motion.div>
                    ...
                    <Link href={cat.href} className="outline-none group block h-full">
                      <div className={`bg-white/[0.02] backdrop-blur-2xl border border-white/[0.06] shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] rounded-3xl p-8 transition-all duration-500 h-full min-h-[240px] flex flex-col justify-between ${cat.glow}`}>
                        <div>
                          ...
                          <h2 className="text-xl font-bold text-white mb-2 group-hover:text-purple-300 transition-colors">{cat.nombre}</h2>
