      ...
      <div className="w-full max-w-7xl">
        <div className="flex items-center gap-3 mb-6 pb-4 border-b border-white/5">
          <div className="bg-gradient-to-tr from-amber-500/30 to-yellow-500/20 p-3 rounded-2xl border border-amber-400/40 shadow-[0_0_20px_rgba(245,158,11,0.3)]">
            <RotateCw className="w-7 h-7 text-amber-300" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <Link href="/organizar" className="text-xs font-bold text-slate-400 hover:text-amber-400 transition-colors">
                {isEs ? 'Organizar PDF' : 'Organize PDF'}
              </Link>
              <span className="text-slate-600 text-xs">/</span>
              <span className="text-amber-400 text-xs font-bold">{isEs ? 'Rotar PDF' : 'Rotate PDF'}</span>
            </div>
