...
const websiteSchema = {
  ...
  featureList: [
    'Editar PDF', 'Comprimir PDF', 'Unir PDF', 'Dividir PDF', 'Firmar PDF',
    'OCR PDF', 'Proteger PDF (AES-256)', 'Convertir PDF a Word/Excel/PowerPoint',
    'Procesamiento 100% local', 'Sin registro', 'Sin límite de tamaño',
  ],
};
...
function AnimatedCounter({ from = 0, to, decimals = 0, suffix = '' }: { from?: number, to: number, decimals?: number, suffix?: string }) {
  ...
  return <span ref={nodeRef} aria-live="polite">{from.toFixed(decimals)}{suffix}</span>;
}
...
export default function DashboardPage() {
  ...
  return (
    ...
    >
          ...
          >
            <div className="w-full h-full max-w-5xl max-h-[80vh] border-2 border-white/30 border-dashed rounded-3xl flex flex-col items-center justify-center bg-zinc-900/60 pointer-events-none shadow-2xl">
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 2 }}>
                <UploadCloud className="w-24 h-24 text-white mb-6 drop-shadow-[0_0_25px_rgba(255,255,255,0.2)]" aria-hidden="true" />
              </motion.div>
      ...
      {isMounted && (
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden flex justify-center items-start" aria-hidden="true">
      ...
      <div className="w-full max-w-7xl relative">
        <input type="file" accept=".pdf" className="hidden" ref={fileInputRef} onChange={handleFileInput} aria-label={isEs ? 'Seleccionar archivo PDF' : 'Select PDF file'} />
        ...
        <AnimatePresence mode="wait">
          ...
          {!isUploading && (
            ...
            >
              ...
              <div className="mb-6 text-center md:text-left">
                ...
                <p className="text-zinc-600 dark:text-zinc-300 text-xs sm:text-sm font-mono mt-4 leading-normal truncate">
                  {isEs 
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch mb-6">
                ...
                <div className="lg:col-span-5 flex flex-col h-full">
                  {!file ? (
                    <div 
                      ...
                      tabIndex={0}
                      role="button"
                      aria-label={isEs ? 'Zona de carga de archivos PDF. Haz clic o arrastra un archivo.' : 'PDF upload area. Click or drag a file.'}
                    ...
                    >
                      <span id="dropzone-instructions" className="sr-only">
                        {isEs ? 'Arrastra un archivo PDF a esta zona o haz clic para seleccionar un archivo de tu equipo. Solo se aceptan archivos PDF.' : 'Drag a PDF file to this area or click to select a file from your device. Only PDF files are accepted.'}
                      </span>
                      ...
                      <div className="text-center flex flex-col items-center gap-2">
                        ...
                        <p className="text-zinc-500 dark:text-zinc-400 text-xs font-mono flex items-center justify-center gap-1.5">
                          <Sparkles className="w-3.5 h-3.5 text-zinc-400 dark:text-zinc-300" aria-hidden="true" />
                      ...
                      <span className="flex items-center justify-center gap-2 bg-white text-black hover:bg-zinc-200 px-6 py-2.5 rounded-full font-sans text-xs font-semibold transition-all mt-2 shadow-md">
                        <FilePlus className="w-4 h-4 text-black" aria-hidden="true" /> {isEs ? "Seleccionar PDF" : "Select PDF"}
                      </span>
                    ...
                    /* VISOR DEL PDF */
                    <div className="w-full flex-1 h-full min-h-[480px] bg-[#09090b] border border-white/20 rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 flex flex-col relative" role="region" aria-label={isEs ? 'Vista previa del PDF cargado' : 'Loaded PDF preview'}>
                      <div className="bg-zinc-900 border-b border-white/10 p-4 flex justify-between items-center z-10 font-mono">
                        <div className="flex items-center gap-3 overflow-hidden">
                          <div className="bg-white/10 p-2 border border-white/10 rounded-lg flex-shrink-0" aria-hidden="true">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button 
                            ...
                            aria-label={isEs ? `Quitar archivo ${file.name}` : `Remove file ${file.name}`}
                          >
                ...
                <div className="lg:col-span-7 flex flex-col justify-between">
                  {file && (
                    <div className="mb-3 flex items-center gap-2.5 bg-white/10 border border-white/20 text-white px-4 py-2.5 rounded-xl shadow-lg font-mono" role="status" aria-live="polite">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 h-full">
                    {categories.map((cat) => (
                      <Link key={cat.id} href={cat.path} className="outline-none group" aria-label={`${isEs ? cat.titleEs : cat.titleEn} — ${isEs ? cat.descEs : cat.descEn}`}>
                        <motion.div 
                        ...
                        >
                          <div>
                            <div className="mb-3 flex items-center justify-between font-mono">
                              ...
                              {cat.badgeEs && (
                                <span className="px-2 py-0.5 text-[9px] font-semibold border border-white/10 bg-zinc-900 text-zinc-400 rounded-full" aria-label={isEs ? cat.badgeEs : cat.badgeEn}>
                            ...
                            <div className="grid grid-cols-2 gap-1.5 mb-2 font-mono" role="list" aria-label={isEs ? 'Herramientas disponibles' : 'Available tools'}>
                              {(isEs ? cat.toolsListEs : cat.toolsListEn).map((toolName, tIdx) => (
                                <div key={tIdx} className="bg-zinc-900/80 border border-white/5 rounded-md px-2 py-1 text-[10px] text-zinc-400 group-hover:text-zinc-200 transition-colors truncate" role="listitem">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3 my-6 font-mono" role="region" aria-label={isEs ? 'Estadísticas de sesión' : 'Session statistics'}>
                ...
                <KpiPill icon={FileText} title={isEs ? "Archivos" : "Files"} value={filesProcessed} tooltip={isEs ? "Tus archivos procesados esta semana" : "Files processed this week"} color="text-white" />
                <KpiPill icon={HardDrive} title={isEs ? "Ahorrado" : "Saved"} value={bytesSaved / (1024 * 1024)} decimals={1} suffix=" MB" tooltip={isEs ? "Almacenamiento optimizado localmente" : "Locally optimized storage"} color="text-zinc-300" />
                <KpiPill icon={Clock} title={isEs ? "Tiempo" : "Time"} value={timeSavedMinutes} suffix=" min" tooltip={isEs ? "Tiempo ahorrado en tu sesión actual" : "Time saved in current session"} color="text-zinc-400" />
              </div>

              {/* SECCIÓN 4 PASOS STYLE CONTENT ARCHITECTURE */}
              {!file && (
                <section className="w-full mt-14 pt-12 border-t border-white/10 flex flex-col items-center font-mono" aria-label={isEs ? 'Cómo funciona PDFBlack' : 'How PDFBlack works'}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 w-full">
                    {/* PASO 1 */}
                    <article className="flex flex-col items-start p-6 bg-[#09090b] border border-white/10 rounded-2xl hover:border-white/30 transition-all group" aria-labelledby="step-1-title">
                      <span className="text-xs text-zinc-500 font-bold mb-3">001 / CARGAR</span>
                      <h3 id="step-1-title" className="text-sm font-bold text-white mb-1.5 font-sans">
                    ...
                    {/* PASO 2 */}
                    <article className="flex flex-col items-start p-6 bg-[#09090b] border border-white/10 rounded-2xl hover:border-white/30 transition-all group" aria-labelledby="step-2-title">
                      <span className="text-xs text-zinc-500 font-bold mb-3">002 / CATEGORÍA</span>
                      <h3 id="step-2-title" className="text-sm font-bold text-white mb-1.5 font-sans">
                    ...
                    {/* PASO 3 */}
                    <article className="flex flex-col items-start p-6 bg-[#09090b] border border-white/10 rounded-2xl hover:border-white/30 transition-all group" aria-labelledby="step-3-title">
                      <span className="text-xs text-zinc-500 font-bold mb-3">003 / EDICIÓN</span>
                      <h3 id="step-3-title" className="text-sm font-bold text-white mb-1.5 font-sans">
                    ...
                    {/* PASO 4 */}
                    <article className="flex flex-col items-start p-6 bg-[#09090b] border border-white/10 rounded-2xl hover:border-white/30 transition-all group" aria-labelledby="step-4-title">
                      <span className="text-xs text-zinc-500 font-bold mb-3">004 / DESCARGAR</span>
                      <h3 id="step-4-title" className="text-sm font-bold text-white mb-1.5 font-sans">
                  ...
                  <section className="w-full mt-14 pt-12 border-t border-white/10 font-sans" aria-label={isEs ? 'Guía técnica de grupos de herramientas' : 'Technical tool group guide'}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full text-left">
                      ...
                      {/* GRUPO 1: EDITAR */}
                      <article className="bg-[#09090b] border border-white/10 hover:border-white/30 rounded-2xl p-6 lg:p-8 transition-all shadow-2xl flex flex-col justify-between" aria-labelledby="group-edit-title">
                        <div>
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="bg-zinc-900 p-3 rounded-xl border border-white/10" aria-hidden="true">
                                <Edit3 className="w-5 h-5 text-white" />
                              </div>
                              <div>
                                <span className="text-xs font-mono text-zinc-500 font-bold block">001 / EDICIÓN DIRECTA</span>
                                <h4 id="group-edit-title" className="text-xl font-bold text-white tracking-tight">
                          ...
                          <ul className="space-y-2 text-xs text-zinc-300 font-mono mb-6" aria-label={isEs ? 'Herramientas de edición' : 'Editing tools'}>
                        ...
                        <Link href="/editar" className="inline-flex items-center justify-between bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-white font-mono text-xs px-4 py-2.5 rounded-xl transition-all group" aria-label={isEs ? 'Ver todas las herramientas de editar PDF' : 'View all edit PDF tools'}>
                          <span>{isEs ? 'Ver herramientas de Editar →' : 'View Edit tools →'}</span>
                          <ArrowRight className="w-3.5 h-3.5 text-white group-hover:translate-x-1 transition-transform" aria-hidden="true" />
                        </Link>
                      </article>

                      {/* GRUPO 2: ORGANIZAR */}
                      <article className="bg-[#09090b] border border-white/10 hover:border-white/30 rounded-2xl p-6 lg:p-8 transition-all shadow-2xl flex flex-col justify-between" aria-labelledby="group-organize-title">
                        <div>
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="bg-zinc-900 p-3 rounded-xl border border-white/10" aria-hidden="true">
                              ...
                              <div>
                                ...
                                <h4 id="group-organize-title" className="text-xl font-bold text-white tracking-tight">
                          ...
                          <ul className="space-y-2 text-xs text-zinc-300 font-mono mb-6" aria-label={isEs ? 'Herramientas de organización' : 'Organization tools'}>
                            <li className="flex items-start gap-2">
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
                        ...
                        <Link href="/organizar" className="inline-flex items-center justify-between bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-white font-mono text-xs px-4 py-2.5 rounded-xl transition-all group" aria-label={isEs ? 'Ver todas las herramientas de organizar PDF' : 'View all organize PDF tools'}>
                      ...
                      {/* GRUPO 3: CONVERTIR */}
                      <article className="bg-[#09090b] border border-white/10 hover:border-white/30 rounded-2xl p-6 lg:p-8 transition-all shadow-2xl flex flex-col justify-between" aria-labelledby="group-convert-title">
                        <div>
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="bg-zinc-900 p-3 rounded-xl border border-white/10" aria-hidden="true">
                              ...
                              <div>
                                ...
                                <h4 id="group-convert-title" className="text-xl font-bold text-white tracking-tight">
                          ...
                          <ul className="space-y-2 text-xs text-zinc-300 font-mono mb-6" aria-label={isEs ? 'Herramientas de conversión' : 'Conversion tools'}>
                            <li className="flex items-start gap-2">
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
                        ...
                        <Link href="/convertir" className="inline-flex items-center justify-between bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-white font-mono text-xs px-4 py-2.5 rounded-xl transition-all group" aria-label={isEs ? 'Ver todas las herramientas de convertir PDF' : 'View all convert PDF tools'}>
                      ...
                      {/* GRUPO 4: OPTIMIZAR */}
                      <article className="bg-[#09090b] border border-white/10 hover:border-white/30 rounded-2xl p-6 lg:p-8 transition-all shadow-2xl flex flex-col justify-between" aria-labelledby="group-optimize-title">
                        <div>
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div className="bg-zinc-900 p-3 rounded-xl border border-white/10" aria-hidden="true">
                              ...
                              <div>
                                ...
                                <h4 id="group-optimize-title" className="text-xl font-bold text-white tracking-tight">
                          ...
                          <ul className="space-y-2 text-xs text-zinc-300 font-mono mb-6" aria-label={isEs ? 'Herramientas de optimización' : 'Optimization tools'}>
                            <li className="flex items-start gap-2">
                              <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
                        ...
                        <Link href="/optimizar" className="inline-flex items-center justify-between bg-zinc-900 hover:bg-zinc-800 border border-white/10 text-white font-mono text-xs px-4 py-2.5 rounded-xl transition-all group" aria-label={isEs ? 'Ver todas las herramientas de optimizar PDF' : 'View all optimize PDF tools'}>
          ...
          {isUploading && (
            <motion.div 
              ...
              role="status"
              aria-live="polite"
              aria-label={isEs ? `Cargando documento: ${uploadProgress}%` : `Loading document: ${uploadProgress}%`}
            >
              <div className="max-w-md mx-auto">
                <div className="flex justify-between items-end mb-4 font-sans">
                  ...
                  <span className="text-white font-bold text-3xl tabular-nums font-mono" aria-live="polite">{uploadProgress}%</span>
                </div>
                <div className="w-full bg-zinc-900 rounded-full h-3 overflow-hidden border border-white/10" role="progressbar" aria-valuenow={uploadProgress} aria-valuemin={0} aria-valuemax={100} aria-label={isEs ? 'Progreso de carga' : 'Upload progress'}>
                  <motion.div 
        ...
        <section className="relative z-10 mt-12 sm:mt-16 font-sans" aria-label={isEs ? 'Archivos recientes' : 'Recent files'}>
          <div className="w-full bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl mb-12">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-white/10 pb-5">
              <div className="flex items-center gap-3">
                <div className="bg-zinc-900 p-2.5 rounded-xl border border-white/10" aria-hidden="true">
              ...
              <div className="relative w-full sm:w-72 font-mono">
                ...
                <label htmlFor="file-search" className="sr-only">{isEs ? 'Buscar archivos' : 'Search files'}</label>
                <input 
                  id="file-search"
                  type="text" 
            ...
            <div className="overflow-x-auto" role="table" aria-label={isEs ? 'Lista de archivos recientes' : 'Recent files list'}>
              <table className="w-full text-left text-xs font-mono whitespace-nowrap">
                <thead>
                  <tr className="border-b border-white/10">
                    <th scope="col" className="pb-3 font-semibold text-zinc-400 uppercase tracking-wider pl-2">{isEs ? 'NOMBRE DEL ARCHIVO' : 'FILE NAME'}</th>
                    <th scope="col" className="pb-3 font-semibold text-zinc-400 uppercase tracking-wider">{isEs ? 'TAMAÑO' : 'SIZE'}</th>
                    <th scope="col" className="pb-3 font-semibold text-zinc-400 uppercase tracking-wider">{isEs ? 'ACCIÓN REALIZADA' : 'ACTION PERFORMED'}</th>
                    <th scope="col" className="pb-3 font-semibold text-zinc-400 uppercase tracking-wider">{isEs ? 'ESTADO' : 'STATUS'}</th>
                    <th scope="col" className="pb-3 font-semibold text-zinc-400 uppercase tracking-wider text-right pr-2">{isEs ? 'ACCIONES' : 'ACTIONS'}</th>
                  </tr>
                </thead>
...
function KpiPill({ icon: Icon, title, value, decimals = 0, suffix = '', tooltip, color }: { icon: React.ElementType; title: string; value: number; decimals?: number; suffix?: string; tooltip?: string; color?: string }) {
  return (
    <div className="relative group/kpi">
      <div className="flex items-center gap-2 px-3.5 py-1.5 bg-zinc-900 border border-white/10 hover:border-white/30 rounded-full transition-all cursor-help font-mono" role="status" aria-label={`${title}: ${value}${suffix}${tooltip ? ` — ${tooltip}` : ''}`}>
      ...
      {tooltip && (
        <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 px-3 py-1.5 bg-zinc-900 border border-white/10 rounded-xl text-[10px] font-mono text-zinc-300 opacity-0 group-hover/kpi:opacity-100 transition-opacity duration-200 pointer-events-none shadow-2xl whitespace-nowrap z-50" aria-hidden="true">
          {tooltip}
        </div>
      )}
    </div>
  ...
  return (
    <tr className="border-b border-white/10 hover:bg-zinc-900/40 transition-colors group">
      <td className="py-3.5 pl-2">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-zinc-900 rounded-xl border border-white/10 group-hover:border-white/30 transition-colors" aria-hidden="true">
      ...
      <td className="py-3.5 pr-2 text-right">
        <div className="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity duration-200">
          <button className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer" aria-label={isEs ? `Añadir ${name} a favoritos` : `Add ${name} to favorites`}><Star className="w-3.5 h-3.5" aria-hidden="true" /></button>
          <button className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer" aria-label={isEs ? `Vista previa de ${name}` : `Preview ${name}`}><Eye className="w-3.5 h-3.5" aria-hidden="true" /></button>
          <button className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer" aria-label={isEs ? `Descargar ${name}` : `Download ${name}`}><Download className="w-3.5 h-3.5" aria-hidden="true" /></button>
          <button className="p-1.5 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer" aria-label={isEs ? `Eliminar ${name}` : `Delete ${name}`}><Trash2 className="w-3.5 h-3.5" aria-hidden="true" /></button>
        </div>
      </td>
    </tr>
  );
