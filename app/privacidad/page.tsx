    ...
    <div className="w-full min-h-screen bg-[#09090b] text-white px-4 sm:px-6 lg:px-8 py-12 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 font-mono">
          ...
          >
            <div
              className="p-3 rounded-2xl bg-zinc-900 text-white border border-white/10 mb-4"
              aria-hidden="true"
            >
          ...
          >
            <div
              className="p-3 rounded-2xl bg-zinc-900 text-emerald-400 border border-white/10 mb-4"
              aria-hidden="true"
            >
        ...
        <div
          ...
          role="article"
          aria-label={
            isEs ? 'Texto completo de la política de privacidad' : 'Full privacy policy text'
          }
        >
          <section className="space-y-3" aria-labelledby="sec-1">
            ...
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
                ? 'El responsable del tratamiento de los datos personales recabados a través de este sitio web es PDFBlack. Puedes contactarnos en cualquier momento a través del correo electrónico contacto@pdf-black.com o mediante el formulario de contacto disponible en la página /contacto.'
                : 'The data controller for personal data collected through this website is PDFBlack. You can contact us at any time via email at contacto@pdf-black.com or through the contact form on the /contacto page.'}
            </p>
          </section>
            ...
            <div className="space-y-3 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p>
                <strong className="text-white">
                  {isEs
                    ? '2.1 Datos de registro (base legal: consentimiento explícito, Art. 6(1)(a) GDPR):'
                    : '2.1 Registration data (legal basis: explicit consent, Art. 6(1)(a) GDPR):'}
                </strong>{' '}
                {isEs
              ...
              <p>
                <strong className="text-white">
                  {isEs
                    ? '2.2 Datos técnicos (base legal: interés legítimo, Art. 6(1)(f) GDPR):'
                    : '2.2 Technical data (legal basis: legitimate interest, Art. 6(1)(f) GDPR):'}
                </strong>{' '}
                {isEs
              ...
              <p>
                <strong className="text-white">
                  {isEs
                    ? '2.3 Cookies y almacenamiento local (base legal: consentimiento, Art. 6(1)(a) GDPR):'
                    : '2.3 Cookies and local storage (legal basis: consent, Art. 6(1)(a) GDPR):'}
                </strong>{' '}
                {isEs
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="sec-3">
            ...
            >
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs
                ? '3. Procesamiento de Archivos PDF — Cero Transmisión'
                : '3. PDF File Processing — Zero Transmission'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="sec-5">
            ...
            >
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs
                ? '5. Seguridad de los Datos — Medidas Técnicas y Organizativas'
                : '5. Data Security — Technical and Organizational Measures'}
            </h2>
            <div className="space-y-3 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p>
                {isEs
              ...
              <ul
                ...
                aria-label={isEs ? 'Lista de medidas de seguridad' : 'List of security measures'}
              >
                <li>
                  {isEs
                    ? 'Cifrado AES-256 para protección de PDFs según ISO 32000-2:2020 (PDF 2.0).'
                    : 'AES-256 encryption for PDF protection per ISO 32000-2:2020 (PDF 2.0).'}
                </li>
                <li>
                  {isEs
                    ? 'Procesamiento exclusivo en RAM sin persistencia en disco.'
                    : 'RAM-only processing with no disk persistence.'}
                </li>
                <li>
                  {isEs
                    ? 'Comunicaciones exclusivamente mediante HTTPS/TLS 1.3.'
                    : 'Communications exclusively via HTTPS/TLS 1.3.'}
                </li>
                <li>
                  {isEs
                    ? 'Headers de seguridad HTTP: Content-Security-Policy, X-Frame-Options, HSTS, X-Content-Type-Options.'
                    : 'HTTP security headers: Content-Security-Policy, X-Frame-Options, HSTS, X-Content-Type-Options.'}
                </li>
                <li>
                  {isEs
                    ? 'Aislamiento de procesos en Web Workers (sin acceso al DOM principal).'
                    : 'Process isolation via Web Workers (no access to main DOM).'}
                </li>
                <li>
                  {isEs
                    ? 'Censura binaria real (eliminación física del código del documento).'
                    : 'True binary redaction (physical removal from document code).'}
                </li>
              </ul>
            </div>
          </section>
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="sec-7">
            ...
            >
              ...
              {isEs
                ? '7. CCPA/CPRA — Derechos de Privacidad en California'
                : '7. CCPA/CPRA — California Privacy Rights'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
