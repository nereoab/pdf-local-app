...
import { ShieldAlert, FileText, ArrowLeft, CheckCircle2, AlertTriangle, Scale, Info } from 'lucide-react';
  ...
  return (
    <div className="w-full min-h-screen bg-[#09090b] text-white px-4 sm:px-6 lg:px-8 py-12 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-10 shadow-2xl space-y-8 font-sans" role="article" aria-label={isEs ? 'Términos y condiciones completos' : 'Full terms and conditions'}>
          
          <section className="space-y-3" aria-labelledby="terms-1">
            <h2 id="terms-1" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '1. Aceptación de los Términos' : '1. Acceptance of Terms'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
                ? 'El acceso y uso del sitio web pdfblack.com (en adelante, "el Sitio") y de cualquiera de sus herramientas de procesamiento de archivos PDF (en adelante, "el Servicio") atribuye la condición de usuario y implica la aceptación plena y sin reservas de todas las disposiciones incluidas en estos Términos y Condiciones, en la Política de Privacidad, en el Aviso Legal, y en el Acuerdo de Procesamiento de Datos (DPA).'
                : 'Access and use of the website pdfblack.com (hereinafter, "the Site") and any of its PDF file processing tools (hereinafter, "the Service") confers the status of user and implies full and unreserved acceptance of all provisions included in these Terms and Conditions, the Privacy Policy, the Legal Notice, and the Data Processing Agreement (DPA).'}
            </p>
          </section>

          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-2">
            <h2 id="terms-2" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <Scale className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '2. Licencia de Uso y Provisión "Tal Cual" (AS IS)' : '2. License of Use & "As Is" Provision'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs 
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-3">
            <h2 id="terms-3" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
            ...
            <div className="space-y-2 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              ...
              <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm" aria-label={isEs ? 'Lista de exclusiones de responsabilidad' : 'List of liability exclusions'}>
                <li>{isEs ? 'Pérdidas indirectas, daños incidentales, especiales, punitivos o consecuentes.' : 'Indirect, incidental, special, punitive, or consequential damages.'}</li>
                <li>{isEs ? 'Corrupción de datos o archivos corruptos preexistentes.' : 'Data corruption or pre-existing corrupt files.'}</li>
                <li>{isEs ? 'Interrupción de flujos de trabajo o pérdida de productividad.' : 'Workflow disruption or loss of productivity.'}</li>
                <li>{isEs ? 'Pérdida de beneficios, ingresos, datos, fondo de comercio o ahorros anticipados.' : 'Loss of profits, revenue, data, goodwill, or anticipated savings.'}</li>
                <li>{isEs ? 'Daños resultantes de la imposibilidad de uso del Servicio.' : 'Damages resulting from the inability to use the Service.'}</li>
                <li>{isEs ? 'Reclamaciones de terceros relacionadas con el contenido de los archivos procesados.' : 'Third-party claims related to the content of processed files.'}</li>
              </ul>
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-4">
            <h2 id="terms-4" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '4. Responsabilidad del Usuario sobre el Contenido' : '4. User Responsibility for Content'}
            </h2>
            <div className="space-y-2 text-zinc-400 text-xs sm:text-sm leading-relaxed">
              <p>
                {isEs
              ...
              <ul className="list-disc pl-5 space-y-1 text-xs sm:text-sm" aria-label={isEs ? 'Declaraciones del usuario' : 'User representations'}>
                <li>{isEs ? 'Es el propietario legítimo o tiene la autorización correspondiente para manipular, editar, firmar, desbloquear o convertir los archivos PDF procesados.' : 'Is the rightful owner or has proper authorization to manipulate, edit, sign, unlock, or convert processed PDF files.'}</li>
                <li>{isEs ? 'No utilizará el Servicio para desbloquear o manipular archivos de origen ilícito o sin la correspondiente autorización de propiedad intelectual.' : 'Will not use the Service to unlock or manipulate files of illegal origin or without proper intellectual property authorization.'}</li>
                <li>{isEs ? 'No utilizará el Servicio para procesar material ilegal, difamatorio, obsceno, o que viole derechos de terceros.' : 'Will not use the Service to process illegal, defamatory, obscene material, or content that violates third-party rights.'}</li>
                <li>{isEs ? 'Es responsable de mantener copias de seguridad de sus archivos originales antes de procesarlos.' : 'Is responsible for maintaining backups of original files before processing.'}</li>
              </ul>
            </div>
          </section>

          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-5">
            <h2 id="terms-5" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <Info className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '5. Propiedad Intelectual del Servicio' : '5. Intellectual Property of the Service'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-6">
            <h2 id="terms-6" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-7">
            <h2 id="terms-7" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-8">
            <h2 id="terms-8" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <Info className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-9">
            <h2 id="terms-9" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
          ...
          <section className="space-y-3 pt-6 border-t border-white/10" aria-labelledby="terms-10">
            <h2 id="terms-10" className="text-lg font-bold text-white flex items-center gap-2 font-mono">
              <CheckCircle2 className="w-4 h-4 text-emerald-400" aria-hidden="true" />
              {isEs ? '10. Contacto' : '10. Contact'}
            </h2>
            <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
              {isEs
                ? 'Para cualquier pregunta, reclamación o consulta relacionada con estos Términos y Condiciones, puedes contactarnos a través del correo electrónico info@pdfblack.com o mediante el formulario de contacto disponible en la página /contacto del Sitio.'
                : 'For any questions, claims, or inquiries related to these Terms and Conditions, you can contact us via email at info@pdfblack.com or through the contact form available on the /contacto page of the Site.'}
            </p>
          </section>
