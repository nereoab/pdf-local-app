...
export default function DownloadSuccessCard({
  ...
  return (
    ...
    >
      {/* HAZ DE LUZ LÁSER PERIMETRAL EN ORO (BORDER BEAM DORADO ANIMADO) */}
      <div className="absolute -inset-[150%] animate-[spin_5s_linear_infinite] bg-[conic-gradient(from_0deg,transparent_0_310deg,#FBBF24_345deg,#F59E0B_360deg)] pointer-events-none opacity-85" />

      {/* RESPLANDOR AMBIENTAL EN ORO CHAMPAGNE */}
      <div className="absolute -inset-4 bg-gradient-to-r from-amber-500/20 via-yellow-500/10 to-amber-600/20 rounded-3xl blur-2xl pointer-events-none" />
      ...
      <div className="relative w-full bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] rounded-3xl p-6 sm:p-8 space-y-6 font-sans">
        {/* Línea de brillo superior en oro */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-zinc-800 pb-5 relative z-10">
          <div className="flex items-center gap-4">
            <div className="bg-zinc-900 p-3.5 rounded-2xl border border-amber-500/40 shadow-[0_0_20px_rgba(245,158,11,0.25)] flex-shrink-0 flex items-center justify-center">
              <AnimatedCheckmark size={34} />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <span className="flex items-center gap-1.5 px-3 py-1 bg-amber-500/10 border border-amber-500/30 text-amber-300 font-bold text-xs font-mono rounded-full shadow-sm">
                  <Sparkles className="w-3.5 h-3.5 text-amber-400" />
                  {isEs ? 'PROCESO COMPLETADO' : 'PROCESS COMPLETED'}
                </span>
                <span className="flex items-center gap-1.5 px-3 py-1 bg-zinc-900 border border-amber-500/30 rounded-full text-amber-200 text-xs font-mono shadow-sm">
          ...
          {onReset && (
            <button
              onClick={onReset}
              className="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-zinc-200 hover:text-white px-4 py-2.5 rounded-xl text-xs font-mono border border-zinc-700 hover:border-amber-500/50 transition-all cursor-pointer shadow-sm flex-shrink-0"
            >
        ...
        <div className="bg-[#121217] border border-zinc-700/80 rounded-2xl p-5 sm:p-6 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-5 relative z-10 overflow-hidden shadow-inner">
          ...
          {/* BOTÓN PRINCIPAL DE DESCARGA (ORO CHAMPAGNE / TITANIUM GOLD EDITION) */}
          <motion.button
            ...
            className={`relative overflow-hidden flex items-center justify-center gap-3 px-8 py-4 rounded-full font-sans font-extrabold text-sm sm:text-base transition-all cursor-pointer flex-shrink-0 group ${
              downloaded
                ? 'bg-gradient-to-r from-amber-200 via-amber-400 to-yellow-500 text-black border border-amber-300 shadow-[0_0_35px_rgba(251,191,36,0.55)]'
                : 'bg-gradient-to-r from-[#FBBF24] via-[#F59E0B] to-[#D97706] text-black shadow-[0_0_30px_rgba(245,158,11,0.4)] hover:shadow-[0_0_45px_rgba(245,158,11,0.7)] border border-amber-200/70'
            }`}
          >
            {/* Shimmer sweep infinito de luz dorada */}
            ...
            {recommendedTools.map((tool) => {
              ...
              return (
                <button
                  ...
                  className="bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] hover:from-[#22222c] hover:to-[#141419] border border-zinc-700/80 hover:border-amber-400/60 rounded-2xl p-3.5 flex flex-col items-start justify-between gap-3 transition-all duration-300 group text-left cursor-pointer hover:scale-[1.03] shadow-lg relative overflow-hidden"
                >
                  ...
                  <div className="flex items-center justify-between w-full">
                    <div className="p-2 bg-zinc-900 border border-zinc-700 rounded-xl text-white group-hover:border-amber-400/50 group-hover:text-amber-300 group-hover:scale-105 transition-all shadow-sm">
                      <IconComp className="w-4 h-4 text-white group-hover:text-amber-300" />
                    </div>
                    <ArrowRight className="w-3.5 h-3.5 text-zinc-500 group-hover:text-amber-300 group-hover:translate-x-1 transition-all" />
                  </div>
                  <div>
                    <span className="text-xs font-bold text-white block group-hover:text-amber-200 transition-colors font-sans">
