...
export default function HtmlPdfConverter({ defaultMode = 'pdf-to-html' }: HtmlPdfConverterProps) {
  ...
  const loadFilesIntoSlots = useCallback(
    (fileList: FileList | File[], specificSlotIndex?: number) => {
      ...
      const hasHtml = filesArray.some((f) => {
        const n = f.name.toLowerCase();
        return n.endsWith('.html') || n.endsWith('.htm') || n.endsWith('.zip');
      });
      const hasPdf = filesArray.some((f) => f.name.toLowerCase().endsWith('.pdf'));

      if (mode === 'html-to-pdf' && !hasHtml && hasPdf) {
        currentMode = 'pdf-to-html';
        setMode('pdf-to-html');
        toast.info(
          isEs ? 'Modo cambiado automáticamente a PDF a HTML' : 'Switched to PDF to HTML mode',
        );
      } else if (mode === 'pdf-to-html' && !hasPdf && hasHtml) {
        currentMode = 'html-to-pdf';
        setMode('html-to-pdf');
        toast.info(
          isEs ? 'Modo cambiado automáticamente a HTML a PDF' : 'Switched to HTML to PDF mode',
        );
      }
      ...
      for (const f of filesArray) {
        ...
        if (currentMode === 'html-to-pdf' && isHtml) {
          validFiles.push(f);
        } else if (currentMode === 'pdf-to-html' && isPdf) {
          validFiles.push(f);
        }
      }
      ...
      if (validFiles.length === 0) {
        toast.error(
          currentMode === 'html-to-pdf'
            ? isEs
              ? 'Por favor selecciona archivos HTML (.html/.htm)'
              : 'Please select HTML files (.html/.htm)'
            : isEs
        ...
        <div ref={successContainerRef} className="w-full">
          <FoliarSuccessView
            ...
            modeText={
              mode === 'pdf-to-html'
                ? isEs
                  ? 'Motor de Exportación HTML5 Semántico'
                  : 'Semantic HTML5 Export Engine'
                : isEs
            ...
            badgeText={
              mode === 'pdf-to-html'
                ? isEs
                  ? 'Conversión a HTML5 Completada'
                  : 'HTML5 Conversion Completed'
                : isEs
            ...
            successTitle={
              mode === 'pdf-to-html'
                ? isEs
                  ? '¡PDF Convertido a HTML5 con Éxito!'
                  : 'PDF Converted to HTML5 Successfully!'
                : isEs
                  ? '¡HTML Convertido a PDF con Éxito!'
                  : 'HTML Converted to PDF Successfully!'
            }
            downloadButtonText={
              mode === 'pdf-to-html'
                ? isEs
                  ? 'Descargar Archivo HTML (.html)'
            ...
            fallbackUrl={
              mode === 'pdf-to-html'
                ? 'https://pdf-black.com/convertir/pdf-html'
                : 'https://pdf-black.com/convertir/html-pdf'
            }
            metricBadge={
