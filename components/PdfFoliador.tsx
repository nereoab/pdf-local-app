...
export default function PdfFoliador() {
  ...
  }, [file, isEncrypted, isUnlocked, textFormat]);
  ...
  const loadThumbnails = useCallback(
    async (selectedFile: File, pass?: string) => {
      ...
      try {
        ...
        toast.success(
          isEs
            ? `${pdfDoc.numPages} páginas listas para foliado`
            : `${pdfDoc.numPages} pages ready for numbering`,
        );
      } catch (err: any) {
  ...
  const executeFoliado = async () => {
    ...
    try {
      ...
      toast.success(
        isEs ? '¡Documento foliado! Tu archivo está listo.' : 'PDF numbered! Your file is ready.',
      );
    } catch (error: any) {
      console.error(error);
      toast.error(
  ...
  return (
    ...
    >
      ...
      {completedResult ? (
        ...
        >
          {/* BANNER DE RESULTADO Y MÉTRICAS DE FOLIADO (ESTILO PÁGINA DE INICIO) */}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="p-4 bg-zinc-800 border border-zinc-600 rounded-2xl text-white shadow-md">
                  <Hash className="w-7 h-7 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]" />
                </div>
                <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-5 border-t border-zinc-800 text-xs">
              <div className="bg-[#121217] p-4 rounded-2xl border border-zinc-700/80 flex flex-col shadow-inner">
                ...
                <span className="text-white font-bold text-sm font-mono mt-0.5">
                  {Math.max(1, endPage - startPage + 1 - (skipFirstPage ? 1 : 0))}{' '}
                  {isEs ? 'Páginas' : 'Pages'}
                </span>
              </div>
              <div className="bg-[#121217] p-4 rounded-2xl border border-zinc-700/80 flex flex-col shadow-inner">
        ...
        >
          ...
          <p className="text-zinc-400 text-xs sm:text-sm font-mono mb-8 max-w-md">
            {isEs
              ? 'Inserta numeración correlativa y foliados personalizados 100% de forma local.'
              : 'Insert customizable page numbers 100% locally.'}
          </p>
          <button
            type="button"
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-3 border-b border-zinc-800 font-mono text-xs text-zinc-400 font-bold">
              <div className="flex items-center gap-2 text-zinc-200 text-xs font-bold">
                ...
                <span>
                  {isEs
                    ? `001 / VISTA PREVIA (${totalPages} PÁGINAS)`
                    : `001 / PAGES PREVIEW (${totalPages} PAGES)`}
                </span>
              </div>
            ...
            {isLoadingThumbs ? (
              <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] gap-3 font-mono">
                ...
                <p className="text-zinc-400 text-xs">
                  {isEs
                    ? 'Generando vista previa de miniaturas...'
                  ...
                  {(pageThumbnails.length > 0
                    ...
                    return (
                      ...
                      >
                        ...
                        {isIncluded && (
                          <div
                            className={`absolute z-30 ${getDotPositionStyle(position)} transition-all duration-300`}
                          >
          ...
          >
            ...
            <div>
                ...
                <div className="grid grid-cols-3 gap-2 p-2 bg-zinc-950 border border-white/10 rounded-xl shadow-inner">
                  {(
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`w-3 h-3 rounded-full transition-transform ${isSelected ? 'bg-red-600 border-2 border-white scale-110 shadow-[0_0_8px_rgba(239,68,68,0.9)]' : 'bg-zinc-600'}`}
                        />
                      </button>
                    );
                  })}
              ...
              <div className="mb-5 font-mono">
                <label className="text-[11px] font-mono text-zinc-400 uppercase tracking-wider block mb-2">
                  {isEs ? 'Formato de Texto:' : 'Text format:'}
                </label>
                <select
                  value={textFormat}
                ...
                >
                  <option value="only-number">
                    {isEs
                      ? 'Solo número de página (Recomendado)'
                      : 'Only page number (Recommended)'}
                  </option>
                  ...
                  <option value="custom">
                    {isEs ? 'Texto personalizado...' : 'Custom text...'}
                  </option>
                </select>
              ...
              <div className="pt-4 border-t border-white/10 my-4 space-y-4 font-mono">
                ...
                <div>
                  <label className="text-[11px] text-zinc-400 uppercase tracking-wider block mb-2">
                    {isEs ? 'Estilo de Numeración:' : 'Numbering Style:'}
                  </label>
                ...
                <div>
                  <label className="text-[11px] text-zinc-400 uppercase tracking-wider block mb-2">
                    {isEs ? 'Margen del Foliado:' : 'Margin:'}
                  </label>
                  <select
                    value={margin}
                ...
                >
                  <div
                    className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${skipFirstPage ? 'bg-white border-white text-black' : 'border-zinc-600'}`}
                  >
                    {skipFirstPage && <Check className="w-3 h-3 text-black stroke-[3]" />}
                  </div>
                  <span className="text-xs text-zinc-300 font-semibold">
                    {isEs
                      ? 'Omitir numeración en 1ª página (Portada)'
                ...
                <div className="bg-zinc-950 p-3 rounded-xl border border-white/10 space-y-2 font-mono">
                  <label className="text-[10px] text-zinc-400 uppercase tracking-wider block font-bold mb-1">
                    {isEs ? 'METADATOS DEL PDF FOLIADO' : 'NUMBERED PDF METADATA'}
                  </label>
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder={
                        isEs ? 'Ej: Documento_Foliado_2026' : 'Ex: Numbered_Document_2026'
                      }
                      value={docTitle}
