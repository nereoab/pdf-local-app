      ...
      <div className="w-full space-y-8 text-zinc-300 font-sans border-t border-white/10 pt-12 mt-12 mb-12">
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">
                {isEs ? '¿Qué sucede exactamente con tus archivos al unirlos?' : 'What exactly happens to your files during PDF merging?'}
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs sm:text-sm text-zinc-400 leading-relaxed mt-4">
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5 space-y-2">
              ...
              <p>
                {isEs
                  ? 'A diferencia de otros servicios en línea, tus archivos PDF NUNCA se cargan a ningún servidor ni almacenamiento en la nube. La combinación vectorial de objetos y estructuras se ejecuta en tiempo real dentro de la memoria RAM de tu propio navegador web.'
                  : 'Unlike other online services, your PDF files are NEVER uploaded to any server or cloud storage. Vector object merging runs in real time inside your own browser RAM.'}
              </p>
            </div>
            ...
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5 space-y-2">
              ...
              <p>
                {isEs
                  ? 'Una vez finalizada la unión y descargado el archivo unificado, no queda ningún rastro en disco ni en servidores. Al cerrar la pestaña o refrescar la página, el navegador purga por completo el espacio en memoria, garantizando confidencialidad absoluta.'
                  : 'Once merging finishes and you download the file, no traces remain on disk or servers. Closing the tab purges all memory completely, ensuring total confidentiality.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">
                {isEs ? 'El procedimiento técnico de fusión paso a paso' : 'Step-by-step technical merging procedure'}
              </h2>
              <p className="text-xs font-mono text-zinc-400">
                {isEs ? 'Cómo nuestro motor analiza, reordena y compila los árboles binarios de tus PDFs' : 'How our engine analyzes, reorders, and compiles binary PDF object trees'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs">
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              ...
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '1. Inspección de Árboles' : '1. Object Tree Inspection'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Escaneamos los diccionarios de cada PDF individual, validando fuentes, objetos de páginas y referencias cruzadas XRef.'
                  : 'Scans individual PDF dictionaries, validating fonts, page objects, and XRef cross-reference tables.'}
              </p>
            </div>

            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              <span className="text-xs font-bold text-zinc-500 mb-2 block">02 / ESTRUCTURACIÓN</span>
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '2. Reordenamiento y Rangos' : '2. Page Rerouting & Ranges'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Filtramos los rangos de páginas seleccionados, aplicamos orientaciones o tamaños de papel y ajustamos el flujo dúplex.'
                  : 'Filters selected page ranges, applies target orientation or paper size, and adjusts duplex page flow.'}
              </p>
            </div>

            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              <span className="text-xs font-bold text-zinc-500 mb-2 block">03 / ENSAMBLADO</span>
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '3. Carátulas y Numeración' : '3. Separators & Numbering'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Incrustamos páginas en blanco o carátulas de separación entre documentos y agregamos el pie de página numerado.'
                  : 'Embeds blank separator pages or title pages between files and renders the continuous footer page numbers.'}
              </p>
            </div>

            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              <span className="text-xs font-bold text-zinc-500 mb-2 block">04 / DESCARGA</span>
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '4. Compilación PDF 1.7' : '4. PDF 1.7 Compilation'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Generamos un nuevo documento PDF único e indexado compatible con Adobe Acrobat, navegadores y cualquier visor.'
                  : 'Generates a single indexed PDF document compatible with Adobe Acrobat, web browsers, and any viewer.'}
              </p>
            </div>
          </div>
        </div>
