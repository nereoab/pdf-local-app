...
export default function PdfMerger() {
  ...
  const togglePageIncluded = (fileId: string, pageIndex: number) => {
    setFiles((prev) =>
      prev.map((f) => {
        ...
        const updatedDetails = f.pagesDetail.map((p) =>
  ...
  const executeMerge = async () => {
    if (files.length < 2) {
      toast.error(
        isEs
          ? 'Debes agregar al menos 2 archivos PDF para unirlos.'
          : 'You must add at least 2 PDF files to merge.',
      );
      return;
    }
    ...
    if (lockedFile) {
      toast.error(
        isEs
          ? `Por favor ingresa la contraseña de "${lockedFile.file.name}" antes de unir.`
          : `Please enter password for "${lockedFile.file.name}" before merging.`,
      );
      return;
    }
    ...
    try {
      ...
      const filePayloads = await Promise.all(
        files.map(async (item) => ({
          ...
          pagesDetail: item.pagesDetail
            ? item.pagesDetail.map((p) => ({
      ...
      toast.error(
        isEs
          ? `Error al unir los archivos: ${error?.message || 'Error desconocido'}`
          : `Error merging files: ${error?.message || 'Unknown error'}`,
      );
    } finally {
          ...
          <div className="lg:col-span-7 xl:col-span-8 bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] border border-zinc-700/80 hover:border-zinc-500 rounded-3xl p-5 sm:p-6 shadow-2xl flex flex-col lg:h-[760px] lg:max-h-[760px] relative overflow-hidden">
            ...
            <div className="mb-3 px-3 py-1.5 bg-zinc-900/60 border border-white/5 rounded-xl flex items-center justify-between text-[11px] font-mono text-zinc-400">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {isEs
                    ? 'Arrastra y suelta los documentos para cambiar el orden de unión'
                    : 'Drag and drop documents to change merge order'}
                </span>
              </div>
                        ...
                        >
                          ...
                          <div className="flex items-center justify-between mb-2.5">
                            <div className="flex items-center gap-1">
                              <span
                                className="cursor-grab active:cursor-grabbing text-zinc-500 group-hover:text-zinc-300 p-0.5 rounded hover:bg-white/10 transition-colors"
                                title={isEs ? 'Arrastra para reordenar' : 'Drag to reorder'}
                              >
                          ...
                          <div className="w-full aspect-[4/3] bg-zinc-900/80 rounded-xl mb-2.5 flex items-center justify-center overflow-hidden border border-white/5 relative">
                            {item.needsPassword ? (
                              <div className="flex flex-col items-center justify-center p-2 text-center gap-1 bg-amber-950/20 rounded-xl border border-amber-500/30 w-full h-full">
                                ...
                                <div className="flex items-center gap-1 w-full mt-0.5">
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      unlockFileWithPassword(item.id, passwordsMap[item.id] || '')
                                    }
                              ...
                              <img
                                ...
                                alt={item.file.name}
                                className="w-full h-full object-contain p-1.5 rounded-xl bg-white select-none pointer-events-none"
                              />
                            ) : (
                              <div className="flex flex-col items-center gap-1.5 text-zinc-500 font-mono select-none">
                                <FileText className="w-8 h-8 text-zinc-400" />
                                <span className="text-[9px] font-bold uppercase">
                                  {item.pageCount} {isEs ? 'Págs' : 'Pages'}
                                </span>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="space-y-1.5">
                            <h4
                              className="text-white font-bold text-xs truncate max-w-full font-sans"
                            ...
                            <div className="flex items-center justify-between text-[10px] text-zinc-400">
                              ...
                              <span>
                                {item.pageCount} {isEs ? 'páginas' : 'pages'}
                              </span>
                            </div>
                            ...
                            <div className="pt-1.5 border-t border-white/5 flex items-center justify-between gap-1.5 text-[10px]">
                              <button
                                ...
                                title={
                                  isEs
                                    ? 'Inspeccionar y organizar páginas individuales'
                                    : 'Inspect and organize individual pages'
                                }
                              >
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2.5 min-w-0 flex-1">
                            <span
                              className="text-zinc-600 group-hover:text-zinc-300 cursor-grab"
                ...
                <div className="bg-zinc-950/70 p-3.5 rounded-xl border border-white/10 space-y-2.5">
                  ...
                  <label className="flex items-center gap-2.5 text-xs font-bold text-zinc-300 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={duplexMode}
                      onChange={(e) => setDuplexMode(e.target.checked)}
                      className="accent-white w-4 h-4 rounded"
                    />
                    <span>
                      {isEs
                        ? 'Modo Dúplex (Inicio en página impar)'
                ...
                >
                    ...
                    {isLoadingInspectorPages ? (
                      <div className="col-span-full py-16 flex flex-col items-center justify-center gap-3 text-zinc-400">
                        ...
                        <span className="text-xs font-bold">
                          {isEs
                            ? 'Generando vista previa de páginas...'
                  ...
                  <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                    <span className="text-xs text-zinc-400">
                      {isEs
                        ? `${includedCount} páginas seleccionadas para la fusión`
                        : `${includedCount} pages selected for merging`}
                    </span>
                    <button
                      type="button"
                      onClick={() => setInspectingFileId(null)}
