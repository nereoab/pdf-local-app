...
export default function PdfOcr() {
  ...
  const handleCopyText = () => {
    if (extractedText) {
      ...
      setCopied(true);
      toast.success(isEs ? 'Texto copiado' : 'Text copied');
    ...
    try {
      ...
      worker.onmessage = (ev: MessageEvent<OcrWorkerResult>) => {
        ...
        if (msg.type === 'progress') {
          ...
          toast.success(
            isEs
              ? '¡OCR completado! Tu archivo está listo para descargar.'
              : 'OCR completed! Your file is ready for download.',
          );
      ...
      {!file ? (
        ...
        >
          ...
          <p className="text-zinc-400 text-xs sm:text-sm font-mono mb-8 max-w-md">
            {isEs
              ? 'Convierte documentos PDF escaneados en texto seleccionable 100% local.'
              : 'Convert scanned PDFs into searchable documents 100% locally.'}
          </p>
          <button
            type="button"
        ...
        >
          ...
          <div className="bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] border border-zinc-600 rounded-3xl p-6 sm:p-8 shadow-2xl font-mono relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-6 pt-5 border-t border-zinc-800 text-xs">
              ...
              <div className="bg-[#121217] p-4 rounded-2xl border border-zinc-700/80 flex flex-col shadow-inner">
                <span className="text-zinc-400 text-[10px] uppercase font-bold">
                  {isEs ? 'Formato Resultante' : 'Resulting Format'}
                </span>
                <span className="text-white font-bold text-sm font-mono mt-0.5">
                  {completedResult.outputFormat.toUpperCase()}{' '}
                  {completedResult.outputFormat === 'pdf'
                    ? isEs
                      ? '(PDF Buscable)'
                      : '(Searchable PDF)'
                    : ''}
        ...
        >
          ...
          <div
            ...
            className="lg:col-span-7 xl:col-span-7 bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] border border-zinc-700/80 hover:border-zinc-500 rounded-3xl p-6 shadow-2xl flex flex-col justify-between min-h-0 relative overflow-hidden"
          >
            <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-white/25 to-transparent pointer-events-none" />
            {/* CABECERA CON BARRA DE NAVEGACIÓN INTELIGENTE */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3 pb-3 border-b border-zinc-800 font-mono text-xs text-zinc-400 font-bold w-full">
                ...
                {isLoadingThumbs || isRenderingPage ? (
                  <div className="flex flex-col items-center justify-center min-h-[350px] gap-3 font-mono">
                    ...
                    <p className="text-zinc-400 text-xs">
                      {isEs
                        ? `Cargando página ${currentViewPage}...`
                        : `Loading page ${currentViewPage}...`}
                    </p>
                  </div>
                ) : viewerHiResImage ? (
                  <div className="relative aspect-[1/1.414] max-h-full w-auto max-w-full bg-white rounded-xl shadow-2xl flex items-center justify-center overflow-hidden border border-zinc-700 select-none">
                    ...
                    <div className="absolute top-2.5 left-2.5 z-20 bg-zinc-900/90 text-white font-mono font-bold text-[10px] px-2.5 py-1 rounded-md border border-white/10 shadow-md">
                      {isEs
                        ? `Página ${currentViewPage} de ${totalPages}`
            ...
            {extractedText && (
              <div className="mt-3 pt-3 border-t border-zinc-800 font-mono">
                ...
                <textarea
                  readOnly
                  value={extractedText}
                  className="w-full h-24 p-2.5 bg-[#121217] border border-zinc-700/80 rounded-2xl text-xs font-mono text-zinc-300 outline-none resize-none shadow-inner"
                />
              </div>
            )}

            {/* PIE DE VISTA PREVIA CON ESTADO Y ALCANCE */}
            <div className="mt-2 pt-2 border-t border-zinc-800 flex items-center justify-between gap-2 text-[10px] text-zinc-400 font-mono w-full">
              <div className="flex items-center gap-1.5 text-zinc-300">
                <span className="px-2.5 py-0.5 bg-zinc-800 border border-zinc-600 rounded-lg text-zinc-200 font-bold shadow-sm">
                  {selectedPagesSet.has(currentViewPage)
                    ? isEs
                      ? `✓ Esta página será procesada con OCR`
                      : `✓ This page will be processed with OCR`
                    : isEs
                      ? `✗ Esta página está omitida del alcance`
                      : `✗ This page is excluded from scope`}
                </span>
              </div>
              <div className="flex items-center gap-1.5 text-zinc-400 font-bold">
                <span>
                  {isEs
                    ? `Idioma: ${LANG_LABELS[ocrLang]?.es || ocrLang}`
                    : `Language: ${LANG_LABELS[ocrLang]?.en || ocrLang}`}
                </span>
              </div>
            </div>
          </div>
          ...
          <div
            ref={controlPanelRef}
            className="lg:col-span-5 xl:col-span-5 bg-gradient-to-b from-[#18181f] via-[#111116] to-[#0a0a0d] border border-zinc-700/80 hover:border-zinc-500 rounded-3xl p-6 shadow-2xl flex flex-col gap-5 relative overflow-hidden"
          >
            <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-white/25 to-transparent pointer-events-none" />
            {/* Panel title */}
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {(['pdf', 'txt', 'json'] as const).map((fmt) => (
                  ...
                  >
                    ...
                    {fmt === 'json' && <FileCode className="w-4 h-4" />}.{fmt}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="font-mono">
              ...
              <div className="space-y-2">
                {(['all', 'custom'] as const).map((scope) => (
                  <label
                    key={scope}
                    className="flex items-center gap-2.5 text-xs font-bold text-zinc-300 cursor-pointer"
                  >
                    <input
                      type="radio"
                      name="ocr-scope"
                      checked={pageScope === scope}
                      onChange={() => setPageScope(scope)}
            ...
            <div className="font-mono">
              <div className="flex justify-between items-center mb-1">
                <label className="text-[10px] text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  {isEs ? 'Visibilidad Capa de Texto' : 'Text Layer Visibility'}
                </label>
                <span className="text-xs font-bold text-white">
                  {textOpacity === 0 ? (isEs ? 'Invisible' : 'Invisible') : `${textOpacity}%`}
                </span>
              </div>
              <input
                type="range"
              ...
              <span className="text-[9px] text-zinc-400 block mt-0.5">
                {isEs
                  ? '0% = Capa invisible estándar (recomendado)'
                  : '0% = Standard invisible layer (recommended)'}
              </span>
            </div>
              ...
              {[
                {
                  ...
                  val: metaTitle,
                  set: setMetaTitle,
                  ph: isEs ? 'Título del documento...' : 'Document title...',
                },
                {
                  icon: <User className="w-3.5 h-3.5 text-zinc-400" />,
                  label: isEs ? 'Autor' : 'Author',
                  val: metaAuthor,
                  set: setMetaAuthor,
                  ph: isEs ? 'Nombre del autor...' : 'Author name...',
                },
                {
                  ...
                  val: metaSubject,
                  set: setMetaSubject,
                  ph: isEs ? 'Asunto o descripción...' : 'Subject or description...',
                },
            ...
            >
              ...
              <span>
                {isProcessing
                  ? progressMsg
                  : isEs
                    ? `Reconocer Texto (OCR) — ${pageScope === 'all' ? `${totalPages} pág.` : 'Selección'} →`
                    : `Recognize Text (OCR) — ${pageScope === 'all' ? `${totalPages} pages` : 'Selection'} →`}
              </span>
            </button>
          </div>
        </motion.div>
