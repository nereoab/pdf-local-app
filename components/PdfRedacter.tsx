...
import {
  ...
  Square, Eraser, Search, CreditCard, Phone, Mail,
  Type, ZoomIn, ZoomOut, AlertTriangle, Check, SlidersHorizontal,
  ChevronDown, ChevronUp, Shield, Database, Zap, RefreshCw, FilePlus
} from 'lucide-react';
...
export default function PdfRedacter() {
  ...
  // Undo/Redo system
  interface UndoAction { redactions: RedactionBox[]; autoRedactions: RedactionBox[]; }
  const [undoStack, setUndoStack] = useState<UndoAction[]>([]);
  const [redoStack, setRedoStack] = useState<UndoAction[]>([]);
  const thumbnailContainerRef = useRef<HTMLDivElement>(null);
  
  const pushUndo = () => { setUndoStack(prev => [...prev.slice(-49), { redactions: [...redactions], autoRedactions: [...autoRedactions] }]); setRedoStack([]); };
  const handleUndo = () => { if (undoStack.length === 0) return; const prev = undoStack[undoStack.length - 1]; setRedoStack(r => [...r, { redactions: [...redactions], autoRedactions: [...autoRedactions] }]); setRedactions(prev.redactions); setAutoRedactions(prev.autoRedactions); setUndoStack(s => s.slice(0, -1)); };
  const handleRedo = () => { if (redoStack.length === 0) return; const next = redoStack[redoStack.length - 1]; setUndoStack(s => [...s, { redactions: [...redactions], autoRedactions: [...autoRedactions] }]); setRedactions(next.redactions); setAutoRedactions(next.autoRedactions); setRedoStack(s => s.slice(0, -1)); };
        ...
        try {
          ...
          for (const item of textContent.items) {
            if ('str' in item && typeof item.str === 'string' && item.str.trim().length > 0) {
              ...
              extracted.push({
                page: p, str: item.str, vx, vy,
                itemWidth: rawWidth, fontHeight,
  ...
  const runAutoDetection = (textItems: ExtractedTextItem[]) => {
    ...
    patterns.forEach(pattern => {
      ...
      const cat = pattern.category === 'personal_id' ? 'text' : pattern.category === 'financial' ? 'card' : 'email';
  ...
  const findPatternMatchesForPattern = (textItems: ExtractedTextItem[], regex: RegExp, category: 'card' | 'phone' | 'email' | 'text', pattern: SensitivePattern): { matches: SensitiveMatch[], auditEntries: AuditEntry[] } => {
    ...
    textItems.forEach((item, idx) => {
      ...
      while ((match = regex.exec(textStr)) !== null) {
        ...
        matches.push({ id: matchId, page: item.page, category, matchedText: matchedText.length > 30 ? matchedText.slice(0, 28) + '…' : matchedText, redactionBox: { id: `auto-${matchId}`, page: item.page, word: matchedText.slice(0, 20), xPercent: Math.max(0, Math.min(98, xPct)), yPercent: Math.max(0, Math.min(98, yPct)), widthPercent: Math.min(100 - xPct, wPct), heightPercent: Math.min(100 - yPct, hPct) } });
        auditEntries.push({ id: matchId, category: pattern.category, severity: pattern.severity, detectedText: matchedText, page: item.page, xPercent: xPct, yPercent: yPct, action: 'flagged', timestamp: '', patternName: pattern.id });
      }
    });
  ...
  const findPatternMatches = (textItems: ExtractedTextItem[], regex: RegExp, category: 'card' | 'phone' | 'email' | 'text'): SensitiveMatch[] => {
    const matches: SensitiveMatch[] = [];
    textItems.forEach((item, idx) => {
      const textStr = item.str;
      let match: RegExpExecArray | null;
  ...
  const getSubWordMatches = (queryStr: string, textItems: ExtractedTextItem[], isExact: boolean): SensitiveMatch[] => {
    ...
    textItems.forEach((item, idx) => {
      ...
      let startIndex = 0;
      let matchPos = isExact ? textStr.indexOf(query, startIndex) : textLower.indexOf(queryLower, startIndex);
      ...
      while (matchPos !== -1) {
        ...
        matchPos = isExact ? textStr.indexOf(query, startIndex) : textLower.indexOf(queryLower, startIndex);
      }
    });
    return matches;
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      else if (ctrl && e.key === 'f') { e.preventDefault(); const inp = document.querySelector<HTMLInputElement>('input[placeholder*="Escribe la palabra"]'); inp?.focus(); }
      else if (e.key === 'Delete' && activeTool === 'erase') { setRedactions([]); setAutoRedactions([]); }
    };
  ...
  const handleMouseDown = (pageNum: number, e: React.MouseEvent<HTMLDivElement>) => {
    ...
    setDrawPreview({ page: pageNum, xPercent: coords.xPercent, yPercent: coords.yPercent, widthPercent: 0, heightPercent: 0 });
  };
  ...
  const handleMouseUp = (pageNum: number, e: React.MouseEvent<HTMLDivElement>) => {
    if (!isDrawing || activeTool !== 'draw') { setIsDrawing(false); setDrawStart(null); setDrawPreview(null); return; }
    const coords = getPercentCoords(pageNum, e);
  ...
  const handleApplyWordSearch = () => {
    ...
    if (selectedPreset === 'text' && word) {
      ...
      const regex = selectedPreset === 'card' ? /\b(?:\d[ -]*?){12,18}\d\b/g
        : selectedPreset === 'phone' ? /(?:\+?\d{1,4}[\s.-]?)?(?:\(?\d{2,4}\)?[\s.-]?)?\d{3,4}[\s.-]?\d{3,4}[\s.-]?\d{0,4}/g
    ...
    if (newBoxes.length > 0) {
      ...
      toast.success(isEs ? `¡${newBoxes.length} elementos marcados para censura!` : `${newBoxes.length} items marked for redaction!`);
    } else {
  ...
  const censorAllDetected = () => {
    ...
    toast.success(isEs ? `¡${allBoxes.length} datos sensibles marcados para censura!` : `${allBoxes.length} sensitive items marked for redaction!`);
  };
  ...
  const executeRedact = async () => {
    ...
    if (allBoxes.length === 0) {
      toast.warning(isEs ? 'No hay parches de censura para aplicar' : 'No redaction patches to apply');
      return;
    }
    ...
    try {
      ...
      calculateSHA256(fileBuffer).then(hash => {
        ...
        addAuditLogEntry({
          ...
          metadata: { fileName: file.name, fileSize: file.size, redactionCount: allBoxes.length, mode: redactionMode },
        });
      });
      ...
      worker.onmessage = (event: MessageEvent) => {
        ...
        if (msg.type === 'progress') {
          ...
          if (r.mode === 'precision' && (r.stats?.textOperatorsModified ?? 0) === 0 && allBoxes.length > 0) {
            worker.terminate();
  ...
  const handleResult = (r: RedactResult) => {
    ...
    calculateSHA256(r.redactedBytes).then(redactedHash => {
      addCustodyRecord({
        ...
        patternsUsed: auditEntries.map(e => e.patternName).filter((v, i, a) => a.indexOf(v) === i),
  ...
  const applyRasterInline = async (allBoxes: RedactionBox[]) => {
    try {
      setProgressMsg(isEs ? 'Aplicando parches sobre imágenes pre-renderizadas...' : 'Applying patches on pre-rendered images...');
      ...
      if (pagesToProcess) {
        for (let p = 1; p <= totalPages; p++) {
          ...
          setProgressMsg(isEs ? `Procesando página ${p}/${totalPages}...` : `Processing page ${p}/${totalPages}...`);
          ...
          const outBlob = await new Promise<Blob>((res, rej) => canvas.toBlob(b => { if (b) res(b); else rej(new Error('toBlob failed')); }, 'image/jpeg', 0.92));
      ...
      outPdf.setTitle(''); outPdf.setAuthor(''); outPdf.setSubject(''); outPdf.setKeywords([]);
      ...
      const result: RedactResult = {
        ...
        mode: 'raster',
        stats: { precisionPages: 0, rasterPages: redactionsByPage.size, textOperatorsModified: 0, contentPreservedKB: Math.round(pdfBytes.byteLength / 1024) },
      };
      handleResult(result);
    } catch (err) {
    ...
    <div className="w-full max-w-7xl mx-auto flex flex-col gap-4 font-sans">
      ...
      <div className="w-full bg-[#09090b] border border-white/10 rounded-2xl p-4 sm:p-5 mb-2 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-2xl">
        <div className="flex flex-wrap items-center gap-3 sm:gap-4">
          ...
          <div>
            <span className="text-[10px] text-zinc-400 font-mono uppercase tracking-wider block">004 / CENSURA Y REDACCIÓN PERMANENTE DE ARCHIVOS PDF</span>
        ...
        {file && (
          <div className="flex items-center gap-2">
            <div className="bg-zinc-900 border border-white/10 px-3 py-2 rounded-xl text-xs text-white font-mono">
              ...
              <span className="font-bold truncate max-w-[140px] inline-block align-middle">{file.name}</span>
            </div>
            <button onClick={resetRedacter} disabled={isProcessing} className="p-2 bg-zinc-900 hover:bg-red-500/20 text-zinc-400 hover:text-red-400 border border-white/10 rounded-xl transition-all" title={isEs ? 'Quitar archivo' : 'Remove file'}>
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
      ...
      {!file ? (
        <div onClick={() => fileInputRef.current?.click()} className="w-full bg-[#09090b] hover:bg-zinc-900/60 border border-white/10 hover:border-white/30 rounded-2xl p-8 lg:p-14 flex flex-col items-center justify-center gap-6 cursor-pointer transition-all duration-300 group shadow-2xl min-h-[480px] relative overflow-hidden my-4">
          <div className="bg-zinc-900 p-5 rounded-2xl border border-white/10 group-hover:border-white/30 transition-colors"><UploadCloud className="w-12 h-12 text-white" /></div>
          <div className="text-center flex flex-col items-center gap-2 font-sans">
            <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight">{isEs ? 'Arrastra tu PDF aquí para censurar información sensible' : 'Drop your PDF here to redact sensitive info'}</h2>
            <p className="text-zinc-400 text-xs sm:text-sm font-mono">{isEs ? 'O haz clic para explorar tus archivos' : 'Or click to browse your files'}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start mb-6 font-sans">
          ...
          {/* LADO IZQUIERDO: VISOR DE PDF (8 COLUMNAS) */}
          <div className="lg:col-span-8 flex flex-col">
            <div className="w-full bg-[#09090b] border border-white/20 rounded-2xl overflow-hidden shadow-2xl flex flex-col relative font-mono h-[92vh] min-h-[860px]">
              ...
              <div className="bg-zinc-900 border-b border-white/10 px-3.5 py-1.5 flex items-center justify-between font-mono text-xs text-zinc-400 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <button onClick={() => setActiveTool('draw')} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-all cursor-pointer ${activeTool === 'draw' ? 'bg-white text-black border-white font-bold' : 'bg-zinc-800 border-white/10 text-zinc-400 hover:text-white'}`}>
                    <Square className="w-3.5 h-3.5" /><span className="text-[10px] font-semibold">{isEs ? 'Dibujar' : 'Draw'}</span>
                  </button>
                  <button onClick={() => setActiveTool('erase')} className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-all cursor-pointer ${activeTool === 'erase' ? 'bg-red-500 text-white border-red-500 font-bold' : 'bg-zinc-800 border-white/10 text-zinc-400 hover:text-white'}`}>
                    <Eraser className="w-3.5 h-3.5" /><span className="text-[10px] font-semibold">{isEs ? 'Borrar' : 'Erase'}</span>
                  </button>
                  <span className="text-[10px] text-zinc-600 mx-1">|</span>
                  {/* Undo/Redo */}
                  <button onClick={handleUndo} disabled={undoStack.length === 0} className="p-1.5 rounded-lg bg-zinc-800/60 hover:bg-zinc-700 border border-white/10 text-zinc-400 disabled:opacity-30 transition-all cursor-pointer" title={isEs ? 'Deshacer (Ctrl+Z)' : 'Undo (Ctrl+Z)'}>
                    <ArrowLeft className="w-3 h-3" />
                  </button>
                  <button onClick={handleRedo} disabled={redoStack.length === 0} className="p-1.5 rounded-lg bg-zinc-800/60 hover:bg-zinc-700 border border-white/10 text-zinc-400 disabled:opacity-30 transition-all cursor-pointer" title={isEs ? 'Rehacer (Ctrl+Y)' : 'Redo (Ctrl+Y)'}>
                    <ArrowLeft className="w-3 h-3 rotate-180" />
                  </button>
                </div>
                <div className="flex items-center gap-3">
                  <span className="bg-zinc-800 px-2 py-0.5 rounded text-[11px] font-bold text-white">{totalPages} {isEs ? 'Páginas' : 'Pages'}</span>
                  {/* Zoom */}
              ...
              <div ref={scrollContainerRef} className={`flex-1 bg-[#121215] relative overflow-y-auto p-4 ${activeTool === 'draw' ? 'cursor-crosshair' : activeTool === 'erase' ? 'cursor-pointer' : 'cursor-default'}`}>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                  ...
                  return (
                    <div key={pageNum} id={`page-card-${pageNum}`}
                      onMouseDown={(e) => handleMouseDown(pageNum, e)} onMouseMove={(e) => handleMouseMove(pageNum, e)} onMouseUp={(e) => handleMouseUp(pageNum, e)}
                      ...
                      {pageDataUrls[pageNum] ? (
                        <div className="mt-1 w-full flex justify-center relative">
                          <div className="relative w-full max-w-full" data-img-wrapper>
                            <img src={pageDataUrls[pageNum]} alt={`Página ${pageNum}`} className="w-full h-auto rounded shadow-sm border border-gray-200 block" />
                            {liveAutoBoxes.map((box) => (
                              <div key={box.id} style={{ left: `${box.xPercent}%`, top: `${box.yPercent}%`, width: `${box.widthPercent}%`, height: `${box.heightPercent}%` }}
                                ...
                                title={isEs ? `Coincidencia: "${box.word}"` : `Match: "${box.word}"`}>
                                <span className="truncate font-extrabold text-[9px] text-white drop-shadow-md select-none">{box.word}</span>
                                <button onClick={(e) => { e.stopPropagation(); removeRedaction(box.id); }} className="text-white hover:text-red-300 p-0.5 opacity-80 group-hover:opacity-100 transition-opacity ml-1"><X className="w-3 h-3" /></button>
                              </div>
                            ))}
                            {manualBoxes.map((box) => (
                              <div key={box.id} style={{ left: `${box.xPercent}%`, top: `${box.yPercent}%`, width: `${box.widthPercent}%`, height: `${box.heightPercent}%` }}
                                onClick={(e) => { if (activeTool === 'erase') { e.stopPropagation(); handleEraseClick(box.id); } }}
                                ...
                                <button onClick={(e) => { e.stopPropagation(); removeRedaction(box.id); }} className="text-red-400 hover:text-white p-0.5 opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-3 h-3" /></button>
                              </div>
                            ))}
                            {drawPreview && drawPreview.page === pageNum && (
                              <div style={{ left: `${drawPreview.xPercent}%`, top: `${drawPreview.yPercent}%`, width: `${drawPreview.widthPercent}%`, height: `${drawPreview.heightPercent}%` }}
                                className="absolute bg-black/40 border-2 border-dashed border-white rounded-sm z-40 pointer-events-none" />
                            )}
                          </div>
                        </div>
                      ) : (
                        <div className="mt-4 space-y-2"><h2 className="text-xs font-bold text-black font-sans border-b pb-1">PÁGINA {pageNum} - {file.name}</h2><p className="text-[11px] text-gray-800 leading-relaxed font-mono">Cargando representación gráfica...</p></div>
                      )}
                      <div className="mt-6 text-[10px] text-gray-400 border-t pt-1 font-mono flex justify-between"><span>{file.name}</span><span>Pág. {pageNum}</span></div>
                    </div>
                  );
                })}
                </div>
          ...
          {/* LADO DERECHO: PANEL DE CONTROL (4 COLUMNAS) */}
            ...
            <div className="bg-[#09090b] border border-white ring-2 ring-white/20 bg-zinc-900/80 rounded-2xl p-5 flex flex-col justify-between relative shadow-2xl font-sans h-[92vh] min-h-[860px]">
              <div className="flex-1 overflow-y-auto [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden flex flex-col gap-3 font-sans">
                ...
                <div className="flex items-center justify-between mb-1 border-b border-white/10 pb-3 flex-shrink-0">
                  <div>
                    <span className="text-[10px] text-zinc-400 font-mono tracking-wider uppercase font-semibold block mb-0.5">002 / CONFIGURACIÓN</span>
                    <h2 className="text-lg font-bold text-white tracking-tight font-sans uppercase">PANEL DE CONTROL</h2>
                  </div>
                  <div className="bg-zinc-900 p-2 rounded-xl border border-white/10 text-white"><SlidersHorizontal className="w-4 h-4 text-white" /></div>
                </div>
                    ...
                    <>
                      <div className="space-y-1 max-h-[160px] overflow-y-auto mb-2">
                        {sensitiveMatches.slice(0, 15).map((match) => (
                          <div key={match.id} className="flex items-center justify-between text-[10px] bg-zinc-900/60 rounded-lg px-2 py-1 border border-white/5">
                            <div className="flex items-center gap-1.5 truncate flex-1 min-w-0">
                              <span className={`flex-shrink-0 ${match.category === 'card' ? 'text-amber-400' : match.category === 'phone' ? 'text-cyan-400' : match.category === 'email' ? 'text-emerald-400' : 'text-zinc-400'}`}>
                                {match.category === 'card' ? '💳' : match.category === 'phone' ? '📱' : match.category === 'email' ? '✉️' : '📝'}
                              </span>
                        ...
                        {sensitiveMatches.length > 15 && (
                          <p className="text-[9px] text-zinc-500 text-center">+{sensitiveMatches.length - 15} {isEs ? 'más' : 'more'}</p>
                        )}
                      </div>
                      <button onClick={censorAllDetected}
                        ...
                        <span>{isEs ? `Censurar todo (${sensitiveMatches.length})` : `Redact all (${sensitiveMatches.length})`}</span>
                      </button>
                    </>
                  ) : (
                    <p className="text-[10px] text-zinc-500 text-center py-2">{isEs ? 'No se detectaron datos sensibles automáticamente' : 'No sensitive data detected automatically'}</p>
                  )}
                </div>

                {/* === BÚSQUEDA MANUAL === */}
                <div className="font-mono">
                  <span className="text-[11px] text-zinc-400 font-medium">003 / CENSURA Y BÚSQUEDA DE TEXTO</span>
                  <h3 className="text-base font-bold text-white tracking-tight font-sans mt-0.5">{isEs ? 'Censura Manual' : 'Manual Redaction'}</h3>
                </div>
                <div className="relative font-mono">
                  <Search className="w-3.5 h-3.5 text-zinc-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                  <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleApplyWordSearch(); } }}
                ...
                <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                  {(['text', 'card', 'phone', 'email'] as const).map((preset) => (
                    <button key={preset} onClick={() => setSelectedPreset(preset)}
                      ...
                      {preset === 'text' ? <><Type className="w-3.5 h-3.5" /><span>{isEs ? 'Texto Libre' : 'Custom Text'}</span></>
                        : preset === 'card' ? <><CreditCard className="w-3.5 h-3.5 text-amber-400" /><span>💳 CC</span></>
                        : preset === 'phone' ? <><Phone className="w-3.5 h-3.5 text-cyan-400" /><span>📱 Tel</span></>
                        : <><Mail className="w-3.5 h-3.5 text-emerald-400" /><span>✉️ Email</span></>}
                    </button>
                  ))}
                </div>

                <div className="flex justify-end gap-2 font-mono text-xs">
                  <button onClick={() => setSearchQuery('')} className="px-3 py-1.5 bg-zinc-900 border border-white/10 hover:border-white/30 text-zinc-300 rounded-xl transition-all cursor-pointer text-[11px]">
                    {isEs ? 'Cancelar' : 'Cancel'}
                  </button>
                  <button onClick={handleApplyWordSearch} className="px-3.5 py-1.5 bg-white text-black hover:bg-zinc-200 font-bold rounded-xl transition-all cursor-pointer shadow-md flex items-center gap-1.5 text-[11px]">
                    <Check className="w-3.5 h-3.5 text-black" /><span>{isEs ? 'Cubrir Todo' : 'Apply'}</span>
                  </button>
                </div>
                ...
                <div className="mt-1">
                  <button onClick={() => setShowAdvanced(v => !v)}
                    ...
                    <span className="flex items-center gap-2 text-[11px] font-bold text-white tracking-wider">
                      <SlidersHorizontal className="w-3.5 h-3.5 text-white" />{isEs ? 'OPCIONES AVANZADAS' : 'ADVANCED OPTIONS'}
                    </span>
                    ...
                    <div className="mt-2.5 space-y-3.5 bg-zinc-950/60 border border-white/8 rounded-xl p-3.5 font-sans">
                      <div>
                        <label className="text-[10px] font-bold text-zinc-400 block mb-1.5 font-mono tracking-widest uppercase flex items-center gap-1.5">
                          <EyeOff className="w-3 h-3 text-zinc-400" />{isEs ? 'Estilo de Parche' : 'Patch Style'}
                        </label>
                        <div className="grid grid-cols-2 gap-1.5 font-mono text-[10px]">
                          <button onClick={() => setRedactionStyle('black')} className={`py-2 px-2.5 rounded-lg border font-bold transition-all cursor-pointer ${redactionStyle === 'black' ? 'bg-black border-white text-white shadow' : 'bg-zinc-900 border-white/10 text-zinc-400 hover:text-white'}`}>
                            {isEs ? '⬛ Negro Sólido' : '⬛ Solid Black'}
                          </button>
                          <button onClick={() => setRedactionStyle('gray')} className={`py-2 px-2.5 rounded-lg border font-bold transition-all cursor-pointer ${redactionStyle === 'gray' ? 'bg-zinc-700 border-white text-white shadow' : 'bg-zinc-900 border-white/10 text-zinc-400 hover:text-white'}`}>
                            {isEs ? '🩶 Gris' : '🩶 Gray'}
                          </button>
                        </div>
                      </div>
                      <div onClick={() => setExactMatch(v => !v)} className="flex items-center justify-between p-2.5 bg-zinc-900 rounded-xl border border-white/8 cursor-pointer hover:border-white/20 transition">
                        <div>
                          <p className="text-[11px] font-bold text-white">{isEs ? 'Coincidencia exacta' : 'Exact Case Match'}</p>
                          <p className="text-[10px] text-zinc-500 font-mono">{isEs ? 'Sensible a mayúsculas/minúsculas' : 'Case-sensitive'}</p>
                        </div>
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-zinc-400 block mb-1.5 font-mono tracking-widest uppercase flex items-center gap-1.5">
                          <Zap className="w-3 h-3 text-zinc-400" />{isEs ? 'Modo de Censura' : 'Redaction Mode'}
                        </label>
                        <div className="grid grid-cols-2 gap-1.5 font-mono text-[10px]">
                          <button onClick={() => setRedactionMode('precision')}
                            ...
                            title={isEs ? 'Preserva texto no censurado, bookmarks y fuentes. Edita content streams nativos.' : 'Preserves non-redacted text, bookmarks and fonts. Edits native content streams.'}>
                            🎯 {isEs ? 'Precisión' : 'Precision'}
                          </button>
                          <button onClick={() => setRedactionMode('raster')}
                            ...
                            title={isEs ? 'Rasteriza cada página a JPEG. Compatible con cualquier PDF.' : 'Rasterizes each page to JPEG. Compatible with any PDF.'}>
                            📸 {isEs ? 'Raster' : 'Raster'}
                          </button>
                        </div>
                      </div>
                ...
                <AnimatePresence>
                  {isProcessing && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="font-mono">
                ...
                <div className="bg-amber-950/30 border border-amber-500/30 rounded-xl p-3 flex items-start gap-2.5 mt-1">
                  ...
                  <p className="text-[10px] text-amber-200 font-sans leading-snug">
                    {isEs ? 'True Redaction: el contenido censurado se destruye permanentemente. No se puede recuperar.' : 'True Redaction: redacted content is permanently destroyed. Cannot be recovered.'}
                  </p>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-white/10 mt-2 flex-shrink-0">
                {!downloadUrl ? (
                  <button onClick={executeRedact} disabled={isProcessing || (redactions.length + autoRedactions.length) === 0}
                    ...
                    {isProcessing ? (
                      <><Loader2 className="w-4 h-4 animate-spin text-black" /><span>{isEs ? 'Censurando...' : 'Redacting...'}</span></>
                    ) : (
                ...
                <div className="pt-2 flex items-center justify-between font-mono text-xs text-zinc-400">
                  ...
                  <span className="flex items-center gap-1 text-white"><Database className="w-3 h-3" />{isEs ? '100% Local' : '100% Local'}</span>
                </div>
              </div>
            </div>
          </div>
