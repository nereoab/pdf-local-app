      ...
      {!slots.some((s) => s.file !== null) ? (
        ...
        <div ref={successContainerRef} className="w-full">
          <FoliarSuccessView
            ...
            totalPages={completedResult.pagesWithRedactions || 1}
            modeText={
              isEs
                ? 'Motor TrueRedact™ v4.0 de Sanitización Irreversible'
                : 'TrueRedact™ v4.0 Irreversible Sanitization Engine'
            }
            toolName={isEs ? 'Censurar PDF' : 'Redact PDF'}
            badgeText={isEs ? 'Censura Completada' : 'Redaction Completed'}
            successTitle={
              isEs ? '¡Documento Censurado con Éxito!' : 'Document Redacted Successfully!'
            }
            downloadButtonText={isEs ? 'Descargar PDF Censurado' : 'Download Redacted PDF'}
            shareSubject={isEs ? 'documento sanitizado' : 'sanitized document'}
            fallbackUrl="https://pdf-black.com/optimizar/censurar"
            metricBadge={
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-0.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-md font-bold font-mono text-xs">
                  {completedResult.totalRedactions} {isEs ? 'parches aplicados' : 'patches applied'}
                </span>
                <span className="px-2.5 py-0.5 bg-blue-500/10 border border-blue-500/30 text-blue-300 rounded-md font-bold font-mono text-xs">
                  ...
                  {isEs ? 'páginas afectadas' : 'pages affected'}
                </span>
                ...
                {showHashDetails && (
                  <div className="mt-3 pt-3 border-t border-zinc-800/80 grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px] font-mono">
                    <div className="bg-zinc-900/90 p-2 rounded-xl border border-zinc-800/80 flex items-center justify-between gap-2 overflow-hidden">
                      <span className="text-zinc-500 font-bold flex-shrink-0">
                        SHA-256 Original:
                      </span>
                      ...
                      {completedResult.redactedHash && (
                        <button
                          onClick={() => {
                            ...
                            toast.success(
                              isEs ? 'Hash sanitizado copiado' : 'Sanitized hash copied',
                            );
                          }}
