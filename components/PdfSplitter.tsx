...
export default function PdfSplitter() {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto min-h-[calc(100vh-100px)] flex flex-col justify-start">
      ...
      {/* ── GUÍA DE USO: CÓMO DIVIDIR UN PDF ── */}
          ...
          <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-white tracking-tight">
                {isEs ? '¿Cómo dividir o separar un PDF?' : 'How to split or separate a PDF?'}
              </h3>
              <p className="text-xs text-zinc-400 font-mono">
                {isEs ? 'Guía rápida para cortar un PDF en múltiples documentos o extraer páginas específicas.' : 'Quick guide to cut a PDF into multiple documents or extract specific pages.'}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            {[
              { step: '01', titleEs: 'Sube tu PDF', titleEn: 'Upload your PDF', descEs: 'Arrastra el PDF a la zona de carga o haz clic para seleccionarlo. El visor cargará las miniaturas de todas las páginas automáticamente.', descEn: 'Drag the PDF to the upload zone or click to select it. The viewer loads thumbnails of all pages automatically.' },
              { step: '02', titleEs: 'Elige el modo de división', titleEn: 'Choose split mode', descEs: 'Selecciona cómo quieres dividir: por rango de páginas (ej. 1-3, 4-7), una página por archivo, o marcadores de sección definidos en el PDF.', descEn: 'Select how to split: by page range (e.g. 1-3, 4-7), one page per file, or section bookmarks defined in the PDF.' },
              { step: '03', titleEs: 'Configura el formato de salida', titleEn: 'Configure output format', descEs: 'Elige si descargar cada fragmento como PDF individual o empaquetar todos en un archivo .ZIP para descargar de una sola vez.', descEn: 'Choose whether to download each fragment as an individual PDF or package all of them in a .ZIP file for a single download.' },
              { step: '04', titleEs: 'Dividir PDF', titleEn: 'Split PDF', descEs: 'Haz clic en "Dividir PDF →". El motor corta el documento al instante y los fragmentos están listos para descargar localmente.', descEn: 'Click "Split PDF →". The engine cuts the document instantly and fragments are ready for local download.' },
            ].map((item) => (
          ...
          <div className="flex items-start gap-3 mb-5">
            ...
            <div>
              <h3 className="text-lg font-bold text-white tracking-tight">
                {isEs ? '💡 Modos de división disponibles y casos de uso' : '💡 Available split modes and use cases'}
              </h3>
              <p className="text-xs text-zinc-400 font-mono mt-1">
                {isEs ? 'Conoce cuándo usar cada modo para obtener el resultado ideal.' : 'Learn when to use each mode to get the ideal result.'}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-zinc-300">
            {[
              { labelEs: 'Por rangos de páginas', labelEn: 'By page ranges', descEs: 'Define intervalos como "1-3, 4-8, 9-12" para extraer capítulos, secciones o partes específicas de un documento en PDFs separados.', descEn: 'Define intervals like "1-3, 4-8, 9-12" to extract chapters, sections, or specific parts of a document into separate PDFs.' },
              { labelEs: 'Una página por archivo', labelEn: 'One page per file', descEs: 'Extrae cada página del PDF como un documento individual. Ideal para separar hojas de un formulario, facturas o páginas de un catálogo.', descEn: 'Extracts each PDF page as an individual document. Ideal for separating form sheets, invoices, or catalog pages.' },
              { labelEs: 'Descarga en ZIP', labelEn: 'ZIP download', descEs: 'Cuando divides un PDF en muchos fragmentos, todos se empaquetan en un archivo .ZIP que puedes descargar con un solo clic.', descEn: 'When splitting a PDF into many fragments, all are packaged in a .ZIP file you can download with a single click.' },
              { labelEs: 'PDFs con contraseña', labelEn: 'Password-protected PDFs', descEs: 'Si el PDF tiene contraseña de edición, usa primero "Desbloquear PDF" del módulo OPTIMIZAR. Los PDFs con solo contraseña de apertura también deben desbloquearse primero.', descEn: 'If the PDF has an edit password, first use "Unlock PDF" from the OPTIMIZE module. PDFs with only an open password must also be unlocked first.' },
            ].map((tip, i) => (
      ...
      <div className="w-full space-y-8 text-zinc-300 font-sans border-t border-white/10 pt-12 mt-12 mb-12">
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">
                {isEs ? '¿Qué sucede exactamente con tus archivos al dividirlos?' : 'What exactly happens to your files during PDF splitting?'}
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs sm:text-sm text-zinc-400 leading-relaxed mt-4">
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5 space-y-2">
              ...
              <p>
                {isEs
                  ? 'A diferencia de otros servicios en línea, tus archivos PDF NUNCA se cargan a ningún servidor ni almacenamiento en la nube. El corte vectorial y la división de páginas se ejecuta en tiempo real dentro de la memoria RAM de tu propio navegador web.'
                  : 'Unlike other online services, your PDF files are NEVER uploaded to any server or cloud storage. Vector cutting and page splitting run in real time inside your own browser RAM.'}
              </p>
            </div>
            ...
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5 space-y-2">
              ...
              <p>
                {isEs
                  ? 'Una vez divididas y descargadas las partes, no queda ningún rastro en disco ni en servidores. Al cerrar la pestaña o refrescar la página, el navegador purga por completo el espacio en memoria, garantizando confidencialidad absoluta.'
                  : 'Once splitting finishes and you download the parts, no traces remain on disk or servers. Closing the tab purges all memory completely, ensuring total confidentiality.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#09090b] border border-white/10 rounded-2xl p-6 sm:p-8 shadow-2xl">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-white tracking-tight">
                {isEs ? 'El procedimiento técnico de división paso a paso' : 'Step-by-step technical splitting procedure'}
              </h2>
              <p className="text-xs font-mono text-zinc-400">
                {isEs ? 'Cómo nuestro motor analiza, segmenta y extrae páginas de tus PDFs' : 'How our engine analyzes, segment, and extracts pages from your PDFs'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 font-mono text-xs">
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              ...
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '1. Lectura del Árbol' : '1. Tree Reading'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Analizamos la estructura binaria del PDF y mapeamos el índice de referencias XRef para aislar cada página independientemente.'
                  : 'Analyzes the PDF binary structure mapping XRef reference index to isolate every page independently.'}
              </p>
            </div>

            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              <span className="text-xs font-bold text-zinc-500 mb-2 block">02 / SEGMENTACIÓN</span>
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '2. Rangos e Intervalos' : '2. Ranges & Intervals'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Filtramos los rangos de páginas seleccionados, aplicamos cortes por volumen N o separamos hojas pares/impares.'
                  : 'Filters selected page ranges, applies volume cut rules or splits odd/even pages.'}
              </p>
            </div>
            ...
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              ...
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '3. Extracción Vectorial' : '3. Vector Extraction'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Copiamos sin pérdidas los objetos de fuentes, vectores e imágenes de las páginas seleccionadas a nuevos contenedores PDF.'
                  : 'Losslessly copies font, vector, and image objects from selected pages into new PDF containers.'}
              </p>
            </div>
            ...
            <div className="bg-zinc-900/60 p-5 rounded-xl border border-white/5">
              ...
              <h3 className="font-bold text-white text-sm mb-2 font-sans">
                {isEs ? '4. Empaquetado .ZIP' : '4. .ZIP Packaging'}
              </h3>
              <p className="text-zinc-400 text-[11px] leading-relaxed font-sans">
                {isEs
                  ? 'Compilamos cada sub-documento PDF y los agrupamos opcionalmente en un archivo comprimido .ZIP listo para descarga.'
                  : 'Compiles each PDF sub-document and optionally packages them into a compressed .ZIP file ready for download.'}
              </p>
            </div>
          </div>
        </div>
