...
import { Unlock, FileDown, Loader2, X, ShieldCheck, FilePlus, KeyRound } from 'lucide-react';
...
export default function PdfUnlocker() {
  ...
  const executeUnlock = async () => {
    ...
    try {
      ...
      link.href = localUrl;
      link.download = `${originalName}_Desbloqueado.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);

      toast.success(isEs ? '¡Seguridad del PDF removida con éxito!' : 'PDF password security removed successfully!');
    } catch (error) {
      console.error(error);
      toast.error(isEs ? 'No se pudo desbloquear. Verifica si requiere contraseña.' : 'Unable to unlock. Check password if required.');
    } finally {
  ...
  if (!file) {
    return (
      ...
      >
        ...
        >
          <Unlock className="w-16 h-16 text-amber-400 drop-shadow-[0_0_15px_rgba(245,158,11,0.6)]" />
        </motion.div>
        ...
        <div className="text-center flex flex-col items-center gap-1.5">
          ...
          <p className="text-amber-400 text-sm font-semibold flex items-center justify-center gap-1.5">
            {isEs ? 'Elimina contraseñas y restricciones de seguridad de tus archivos PDF' : 'Remove PDF password security giving full freedom'}
          </p>
        </div>
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row gap-6">
      <div className="flex-1 bg-slate-900/80 p-6 rounded-3xl border border-slate-800 min-h-[440px] flex flex-col items-center justify-center relative">
        <div className="absolute top-4 left-4 right-4 bg-slate-800/80 backdrop-blur-sm p-3 rounded-2xl border border-white/10 flex justify-between items-center shadow-sm">
          <div className="flex items-center gap-3 overflow-hidden">
            <Unlock className="w-5 h-5 text-amber-400 flex-shrink-0" />
        ...
        <div className="flex flex-col items-center mt-12">
          <div className="w-36 h-48 bg-amber-950/60 rounded-2xl border-2 border-amber-500/40 flex flex-col items-center justify-center p-4 shadow-xl mb-4">
            <Unlock className="w-14 h-14 text-amber-400 mb-2" />
            <span className="text-xs font-bold text-amber-300 uppercase">SIN RESTRICCIONES</span>
          </div>
        </div>
      </div>
      ...
      <div className="w-full lg:w-80 bg-slate-900/90 border border-slate-800 p-6 rounded-3xl flex flex-col justify-between h-auto lg:h-[440px]">
        <div>
          <h3 className="text-xl font-bold text-white mb-2">{isEs ? 'Desbloquear Seguridad' : 'Unlock Security'}</h3>
          <p className="text-slate-400 text-xs leading-relaxed mb-6">
            {isEs
              ? 'Si el PDF tiene una contraseña de apertura, escríbela a continuación.'
              : 'If PDF has an opening password, type it below.'}
          </p>

          <div className="mb-6">
            <label className="text-xs font-bold text-slate-400 block mb-2">{isEs ? 'Contraseña (Opcional)' : 'Password (Optional)'}</label>
            <div className="relative">
              <input
                ...
                className="w-full p-3 bg-slate-800 border border-white/10 rounded-xl text-white outline-none text-sm focus:border-amber-400"
              />
        ...
        <div className="space-y-3">
          {!downloadUrl ? (
            ...
            >
              {isProcessing ? (
                <>
                  ...
                  <span className="text-sm">{isEs ? 'Desbloqueando...' : 'Unlocking...'}</span>
                </>
              ) : (
                isEs ? 'Desbloquear PDF' : 'Unlock PDF'
              )}
            ...
            >
              ...
              {isEs ? 'Descargar PDF Libre' : 'Download Unlocked PDF'}
            </a>
          )}
        </div>
      </div>
