...
export default function SharedLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen flex-col transition-colors duration-700 selection:bg-blue-500/30 selection:text-blue-200 relative overflow-x-hidden font-sans text-slate-200 bg-black">
      ...
      {/* ENCABEZADO GLOBAL SAAS (Sticky + Glassmorphism + Contraste) */}
      <header className="w-full bg-[#0a0a0a]/80 backdrop-blur-2xl border-b border-white/5 sticky top-0 z-50 transition-all duration-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between relative z-10">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {/* 4. BOTÓN "+ NUEVO" CON MENÚ DESPLEGABLE */}
            <div className="relative group cursor-pointer hidden sm:block">
              <motion.button 
                whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                className="flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-black px-4 py-2 rounded-full font-bold text-sm transition-all shadow-[0_0_15px_rgba(6,182,212,0.4)]"
              >
            ...
            <motion.button onClick={toggleLanguage} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white px-3 py-2 rounded-full font-semibold text-xs transition-all border border-white/5">
  ...
  return (
    <div className="relative group">
      <Link href={basePath} className="flex items-center gap-1 py-6 outline-none">
        <span className={`
          font-black text-[15px] tracking-wide drop-shadow-sm transition-all
        ...
        <ChevronDown className={`w-4 h-4 transition-transform duration-300 group-hover:rotate-180 ${isActive ? 'text-cyan-400' : 'text-slate-400 group-hover:text-white'}`} />
      </Link>
