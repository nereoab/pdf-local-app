          ...
          try {
            ...
            if (response.ok) {
              ...
              if (fileData?.FileData) {
                ...
                const byteNumbers = new Array(byteCharacters.length);
                for (let i = 0; i < byteCharacters.length; i++) byteNumbers[i] = byteCharacters.charCodeAt(i);
                resultBlob = new Blob([new Uint8Array(byteNumbers)], { 
          ...
          <div className="bg-[#09090b] border border-blue-500/30 rounded-2xl p-6 sm:p-8 shadow-2xl relative overflow-hidden font-mono">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-white/10">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  ...
                  <p className="text-zinc-400 text-xs font-mono mt-0.5">
                    {mode === 'word-to-pdf'
                      ? (isEs ? 'Documento Word convertido a PDF vectorial listo para descargar.' : 'Word document converted to vector PDF ready for download.')
                      : (isEs ? 'PDF exportado a documento Word editable con éxito.' : 'PDF exported to editable Word document successfully.')}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-900 border border-white/10 rounded-xl text-xs text-blue-400">
                ...
                <span>{mode === 'word-to-pdf' ? (isEs ? 'PDF Vectorial Listo' : 'Vector PDF Ready') : (isEs ? 'DOCX Editable Listo' : 'Editable DOCX Ready')}</span>
              </div>
            </div>
                  ...
                  <div className="w-28 sm:w-32 flex-shrink-0 bg-zinc-950/90 border-r border-white/10 p-2 overflow-y-auto flex flex-col gap-2.5 custom-scrollbar">
                    ...
                    {isRendering ? (
                      ...
                      <div className="flex flex-col items-center justify-center py-8 gap-2 text-zinc-500 text-[10px] text-center">
                        ...
                        <span>{isEs ? 'Modo DOCX' : 'DOCX Mode'}</span>
                      </div>
                    )}
                  </div>
